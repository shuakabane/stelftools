...
rule authnone_refresh_95da5d637ce2b37c162272462d63ab78 {
	meta:
		aliases = "__pthread_mutexattr_destroy, wcsftime, __pthread_mutex_unlock, pthread_condattr_destroy, pthread_attr_destroy, grantpt, __pthread_mutex_trylock, __GI_pthread_condattr_init, __GI_pthread_attr_destroy, __pthread_return_0, __GI_wcsftime, __pthread_mutex_init, __pthread_mutex_lock, __GI_pthread_condattr_destroy, xdrstdio_inline, pthread_rwlockattr_destroy, clntraw_control, _svcauth_null, pthread_condattr_init, pthread_mutexattr_destroy, authnone_refresh"
		size = "3"
		objfiles = "rwlock@libpthread.a, condvar@libpthread.a, grantpt@libc.a, attr@libpthread.a, svc_auth@libc.a"
	strings:
		$pattern = { ( CC | 31 ) C0 C3 }
	condition:
		$pattern
...
rule restart_7e7c6eb2f9410c7059cbbed1cb6911cf {
	meta:
		...
		size = "8"
		objfiles = "spinlock@libpthread.a, manager@libpthread.a, rwlock@libpthread.a, condvar@libpthread.a, pthread@libpthread.a"
	strings:
...
rule tempnam_42f7b3ea7462a38c0cba2ec10a05c259 {
	meta:
		aliases = "tempnam"
		size = "76"
		objfiles = "tempnam@libc.a"
	strings:
		$pattern = { ( CC | 53 ) 81 EC 00 10 00 00 FF B4 24 0C 10 00 00 FF B4 24 0C 10 00 00 68 FF 0F 00 00 8D 5C 24 0D 53 E8 ?? ?? ?? ?? 83 C4 10 85 C0 75 17 6A 03 53 E8 ?? ?? ?? ?? 59 5A 85 C0 75 09 53 E8 ?? ?? ?? ?? 5A EB 02 31 C0 81 C4 00 10 00 00 5B C3 }
	condition:
		$pattern
}
...
rule login_37631cc927e6b1670c08378b80762f03 {
	meta:
		aliases = "login"
		size = "97"
		objfiles = "login@libutil.a"
	strings:
		$pattern = { ( CC | 53 ) 81 EC 80 01 00 00 8B 9C 24 88 01 00 00 89 E0 68 80 01 00 00 53 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? 66 C7 44 24 10 07 00 E8 ?? ?? ?? ?? 89 44 24 14 6A 20 8D 43 08 50 8D 44 24 20 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 20 E8 ?? ?? ?? ?? 81 C4 80 01 00 00 5B C3 }
	condition:
		$pattern
}
...
rule __GI_getdomainname_f7a619f52b083c41b855d50eb3d96e10 {
	meta:
...
rule __GI_hypot_40d6dc07de69a88c5b186729e931aed9 {
	meta:
...
rule close_593d4632d655ae75a9db77d772cf7bf1 {
	meta:
		aliases = "wait, system, fsync, tcdrain, close"
		size = "43"
		objfiles = "wrapsyscall@libpthread.a"
	strings:
...
rule __GI_nanosleep_cfabef1ece34ce793be11fd3ac6ee861 {
	meta:
...
rule recvmsg_966d33990ea41d94ea545ad487f15973 {
	meta:
		aliases = "__GI_waitpid, waitpid, msync, connect, read, accept, write, lseek, sendmsg, recvmsg"
		size = "51"
		objfiles = "wrapsyscall@libpthread.a"
	strings:
...
rule send_c2940d127b00fb25b0bbd004ee5c830a {
	meta:
		aliases = "recv, pread, pwrite, send"
		size = "55"
		objfiles = "wrapsyscall@libpthread.a"
	strings:
...
rule __GI_xdr_u_long_18d060fd160240e7416349fd6006e170 {
	meta:
...
rule __GI_xdr_u_short_8905bcb326c22100dfa27e7c7ada0bdf {
	meta:
...
rule __GI_xdr_short_03e8595d67a843632b19b3707b0414f4 {
	meta:
...
rule getc_unlocked_e8150f2fa63846a8bbb3e1cc896ad04e {
	meta:
		aliases = "__GI_getc_unlocked, __GI_fgetc_unlocked, __GI___fgetc_unlocked, fgetc_unlocked, __fgetc_unlocked, getc_unlocked"
		size = "204"
...
rule open_8c90e6ff205b5f5400c2e16316f751d2 {
	meta:
...
rule __GI___fpclassify_e4a69a8c76f26553ec406da8d3337a33 {
	meta:
...
rule mbrtowc_dfab0aae07e77cfaf5b8f74aa02c52a7 {
	meta:
		aliases = "__GI_mbrtowc, mbrtowc"
		size = "107"
		objfiles = "mbrtowc@libc.a"
	strings:
		$pattern = { ( CC | 53 ) 83 EC 0C 8B 5C 24 14 8B 44 24 18 8B 54 24 20 85 D2 75 05 BA ?? ?? ?? ?? 85 C0 75 0D C6 44 24 0B 00 31 DB 8D 44 24 0B EB 0C 80 38 00 74 33 83 7C 24 1C 00 74 2C 89 04 24 52 6A 01 6A FF 8D 44 24 0C 50 8D 44 24 14 50 E8 ?? ?? ?? ?? 89 C2 83 C4 14 85 C0 78 0E 85 DB 74 0A 8B 44 24 04 89 03 EB 02 31 D2 89 D0 83 C4 0C 5B C3 }
	condition:
		$pattern
}
...
rule __GI_endnetent_72360ebb209b9039bf09d12a444ae7a0 {
	meta:
		aliases = "endservent, __GI_endservent, endnetent, __GI_endnetent"
		size = "83"
		objfiles = "getservice@libc.a, getnetent@libc.a"
	strings:
		$pattern = { ( CC | 53 ) 83 EC 10 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 5C 24 08 53 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? A1 ?? ?? ?? ?? 83 C4 10 85 C0 74 11 50 E8 ?? ?? ?? ?? C7 05 ?? ?? ?? ?? 00 00 00 00 58 C6 05 ?? ?? ?? ?? 00 6A 01 53 E8 ?? ?? ?? ?? 83 C4 18 5B C3 }
	condition:
		$pattern
}
...
rule endutent_7ec3ebf6b476796ebbd9794f670a411c {
	meta:
		aliases = "endutent"
		size = "76"
		objfiles = "utent@libc.a"
	strings:
		$pattern = { ( CC | 53 ) 83 EC 10 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 5C 24 08 53 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? A1 ?? ?? ?? ?? 83 C4 10 85 C0 78 07 50 E8 ?? ?? ?? ?? 58 C7 05 ?? ?? ?? ?? FF FF FF FF 6A 01 53 E8 ?? ?? ?? ?? 83 C4 18 5B C3 }
	condition:
		$pattern
}

rule endhostent_59e8084ef853dbc0f63a4129aae373b8 {
	meta:
		aliases = "endhostent"
		size = "83"
		objfiles = "gethostent_r@libc.a"
	strings:
		$pattern = { ( CC | 53 ) 83 EC 10 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 5C 24 08 53 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? C6 05 ?? ?? ?? ?? 00 A1 ?? ?? ?? ?? 83 C4 10 85 C0 74 11 50 E8 ?? ?? ?? ?? C7 05 ?? ?? ?? ?? 00 00 00 00 5A 6A 01 53 E8 ?? ?? ?? ?? 83 C4 18 5B C3 }
	condition:
		$pattern
}
...
rule utmpname_ada85e10afad3d607dd87ccef48680a7 {
	meta:
		aliases = "utmpname"
		size = "135"
		objfiles = "utent@libc.a"
	strings:
		$pattern = { ( CC | 53 ) 83 EC 10 8B 5C 24 18 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 10 85 DB 74 2D A1 ?? ?? ?? ?? 3D ?? ?? ?? ?? 74 07 50 E8 ?? ?? ?? ?? 59 53 E8 ?? ?? ?? ?? A3 ?? ?? ?? ?? 5A 85 C0 75 0A C7 05 ?? ?? ?? ?? ?? ?? ?? ?? A1 ?? ?? ?? ?? 85 C0 78 11 50 E8 ?? ?? ?? ?? C7 05 ?? ?? ?? ?? FF FF FF FF 58 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 31 C0 83 C4 18 5B C3 }
	condition:
		$pattern
}
...
rule __GI_gethostent_r_a1f7ac782fa93fc501e73f54459eaddf {
	meta:
		aliases = "gethostent_r, __GI_gethostent_r"
		size = "166"
		objfiles = "gethostent_r@libc.a"
	strings:
		$pattern = { ( CC | 53 ) 83 EC 10 8B 5C 24 24 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 10 83 3D ?? ?? ?? ?? 00 75 1B E8 ?? ?? ?? ?? A3 ?? ?? ?? ?? 85 C0 75 0D C7 03 00 00 00 00 BB 02 00 00 00 EB 46 FF 74 24 28 53 FF 74 24 28 FF 74 24 28 FF 74 24 28 6A 01 6A 02 6A 00 FF 35 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C3 83 C4 24 80 3D ?? ?? ?? ?? 00 75 16 FF 35 ?? ?? ?? ?? E8 ?? ?? ?? ?? C7 05 ?? ?? ?? ?? 00 00 00 00 58 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 89 D8 83 C4 18 5B C3 }
	condition:
		$pattern
}
...
rule __ieee754_sinh_5929730d445d55ac7e265b5785fa5c77 {
	meta:
...
rule __GI_trunc_f26888a12c86391c82f6f272c4966144 {
	meta:
...
rule __GI_signal_fc9fe89dd9159edeaad7e7210c81353b {
	meta:
...
rule __GI_svcerr_noprog_3ccc32a16412d9fd0ea691b4fd7e709b {
	meta:
...
rule __GI_svcerr_progvers_1db920763b141324ad660635e64059b5 {
	meta:
...
rule __GI_asinh_bddfa08f4925cd2bb4f361fb32d28bde {
	meta:
...
rule __GI_vswprintf_2218a44e64b850c531b7ffd1b6a6aeed {
	meta:
...
rule __GI_lstat64_957edea1520df57dac3d603b3e6011ea {
	meta:
...
rule mq_send_8810fe4c8dbaf93db0b2807cdd400643 {
	meta:
		aliases = "pread, __libc_pread, mq_receive, mq_send"
		size = "35"
...
rule __GI_xdrrec_skiprecord_93e08e2a92b88d7240d658fc8038ab48 {
	meta:
...
rule __GI_sigsuspend_4a19b92285673d68e6e8c9d1d53a1e18 {
	meta:
...
rule __GI___cxa_atexit_09b892ba179e1890718127a6775bc4fa {
	meta:
...
rule mkdtemp_d94243ad4e1e94799b962202a4203565 {
	meta:
		aliases = "mkdtemp"
		size = "31"
		objfiles = "mkdtemp@libc.a"
	strings:
		$pattern = { ( CC | 53 ) 8B 5C 24 08 6A 02 53 E8 ?? ?? ?? ?? 5A 59 85 C0 0F 94 C0 0F B6 C0 F7 D8 21 C3 89 D8 5B C3 }
	condition:
		$pattern
}
...
rule mktemp_37342b25382b7aca9470a7604a8c4925 {
	meta:
		aliases = "mktemp"
		size = "26"
		objfiles = "mktemp@libc.a"
	strings:
		$pattern = { ( CC | 53 ) 8B 5C 24 08 6A 03 53 E8 ?? ?? ?? ?? 5A 59 85 C0 79 03 C6 03 00 89 D8 5B C3 }
	condition:
		$pattern
}
...
rule tmpnam_r_3c153b5f4a9ab2cc19c0fdeade4be335 {
	meta:
		aliases = "tmpnam_r"
		size = "48"
		objfiles = "tmpnam_r@libc.a"
	strings:
		$pattern = { ( CC | 53 ) 8B 5C 24 08 85 DB 74 21 6A 00 6A 00 6A 14 53 E8 ?? ?? ?? ?? 83 C4 10 85 C0 75 0E 6A 03 53 E8 ?? ?? ?? ?? 5A 59 85 C0 74 02 31 DB 89 D8 5B C3 }
	condition:
		$pattern
}
...
rule __GI___sigaddset_a0b2dce9ddf206cde67fd512ea066dce {
	meta:
...
rule __pthread_cleanup_push_defer_0589ae6f86c4bdb4ca62d7d8a27c358c {
	meta:
...
rule __GI_nrand48_r_b0d19233fa999b586187102e2fc294ed {
	meta:
...
rule __GI___glibc_strerror_r_814991ee05c5cdbc2b6343e60e7aa014 {
	meta:
...
rule clnttcp_control_ef2bd8672e3f9c1cff65120bc5d548e1 {
	meta:
...
rule clntunix_control_338bd7620bf11499a14c0fbc65fb1577 {
	meta:
...
rule clntudp_control_bdea2758074fc048ec14ab8fdfa0d27d {
	meta:
		aliases = "clntudp_control"
		size = "203"
		objfiles = "clnt_udp@libc.a"
	strings:
		$pattern = { ( CC | 53 ) 8B 5C 24 10 8B 44 24 08 8B 48 08 8B 44 24 0C 48 83 F8 0E 77 07 FF 24 85 ?? ?? ?? ?? 31 C0 E9 A5 00 00 00 C7 41 04 01 00 00 00 E9 94 00 00 00 C7 41 04 00 00 00 00 E9 88 00 00 00 8B 13 8B 43 04 89 41 28 89 51 24 EB 7B 8B 51 24 8B 41 28 EB 13 8B 13 8B 43 04 89 41 20 89 51 1C EB 66 8B 51 1C 8B 41 20 89 43 04 89 13 EB 59 8D 41 08 6A 10 50 53 E8 ?? ?? ?? ?? B8 01 00 00 00 83 C4 0C EB 48 8B 01 EB 31 8B 41 58 8B 00 0F C8 EB 28 8B 03 48 0F C8 8B 51 58 89 02 8B 41 58 8B 40 10 0F C8 EB 14 8B 03 0F C8 8B 51 58 89 42 10 EB 16 8B 41 58 8B 40 0C 0F C8 89 03 EB 0A 8B 03 0F C8 8B 51 58 89 42 0C B8 01 00 00 }
	condition:
		$pattern
}
...
rule __GI_wmemcpy_d1f0e65abe964825a8ca62981a4d12e8 {
	meta:
...
rule __GI_getuid_d186bcc64a7a2c3495678d185e73c4d3 {
	meta:
...
rule __GI_getrpcent_4c7761ee112cc50013e9c035623dd610 {
	meta:
...
rule __GI_authunix_create_default_64d615954a30f1ff01fb1e91b99c8acf {
	meta:
...
rule __GI_clnt_spcreateerror_f0f555ac6720c195bd9a82c3d0211c44 {
	meta:
...
rule __GI_openpty_acce98079e23d9561776536ce58d66e2 {
	meta:
...
rule __GI___res_querydomain_77f0a4bdfee1e4a93cf5407a84267a7e {
	meta:
...
rule __GI_clnt_sperror_1a9d60338ad0cbccafc4a1e0145463ad {
	meta:
...
rule __GI_statvfs_df3c1346be5dc07be898b02e33405ccf {
	meta:
...
rule __GI_vsyslog_3ae5b9dea30b08ffafccb3d04b5ba5f9 {
	meta:
		aliases = "vsyslog, __GI_vsyslog"
		size = "665"
		objfiles = "syslog@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 81 EC 28 04 00 00 8B B4 24 3C 04 00 00 F7 C6 00 FC FF FF 0F 85 71 02 00 00 E8 ?? ?? ?? ?? 89 44 24 08 8B 28 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 84 24 1C 04 00 00 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 0F B6 05 ?? ?? ?? ?? 89 F1 83 E1 07 D3 F8 83 C4 10 A8 01 0F 84 1B 02 00 00 83 3D ?? ?? ?? ?? 00 78 09 80 3D ?? ?? ?? ?? 00 75 18 8A 15 ?? ?? ?? ?? 83 CA 08 0F B6 D2 B9 08 00 00 00 31 C0 E8 ?? ?? ?? ?? F7 C6 F8 03 00 00 75 0C 0F B6 05 ?? ?? ?? ?? C1 E0 03 09 C6 8D 9C 24 24 04 00 00 53 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C0 04 50 56 68 ?? ?? ?? ?? 8D 5C 24 28 53 E8 ?? ?? ?? ?? 01 }
	condition:
		$pattern
}
...
rule _vfprintf_internal_b878abfe8cb4a569dbb5449569aa4615 {
	meta:
		aliases = "_vfprintf_internal"
		size = "1521"
		objfiles = "_vfprintf_internal@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 81 EC 68 01 00 00 8B 9C 24 80 01 00 00 53 8D 74 24 10 56 E8 ?? ?? ?? ?? 5F 5D 85 C0 79 38 8B 5C 24 0C 53 E8 ?? ?? ?? ?? 5E C7 04 24 FF FF FF FF 85 C0 0F 84 A7 05 00 00 FF B4 24 7C 01 00 00 50 53 E8 ?? ?? ?? ?? C7 44 24 0C FF FF FF FF 83 C4 0C E9 89 05 00 00 FF B4 24 84 01 00 00 56 E8 ?? ?? ?? ?? 89 DA C7 44 24 08 00 00 00 00 58 59 EB 01 43 8A 03 84 C0 74 04 3C 25 75 F5 39 D3 74 26 89 DE 29 D6 31 C0 85 F6 7E 11 FF B4 24 7C 01 00 00 56 52 E8 ?? ?? ?? ?? 83 C4 0C 39 F0 0F 85 35 05 00 00 01 04 24 80 3B 00 0F 84 30 05 00 00 8D 53 01 80 7B 01 25 0F 84 14 05 00 00 89 54 24 0C C7 84 24 64 }
	condition:
		$pattern
}
...
rule byte_regex_compile_6e29435c74d6a0e5e7b845811938106f {
	meta:
		aliases = "byte_regex_compile"
		size = "8192"
		objfiles = "regex_old@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 81 EC 78 01 00 00 89 44 24 1C 89 4C 24 18 8B BC 24 8C 01 00 00 89 84 24 74 01 00 00 01 C2 89 54 24 28 8B 47 14 89 44 24 2C 68 80 02 00 00 E8 ?? ?? ?? ?? 89 44 24 68 5D 85 C0 0F 84 9C 1F 00 00 8B 54 24 18 89 57 0C 80 67 1C 97 C7 47 08 00 00 00 00 C7 47 18 00 00 00 00 83 3D ?? ?? ?? ?? 00 75 45 68 00 01 00 00 6A 00 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 31 D2 83 C4 0C EB 14 A1 ?? ?? ?? ?? F6 44 50 01 08 74 07 C6 82 ?? ?? ?? ?? 01 42 81 FA FF 00 00 00 7E E4 C6 05 ?? ?? ?? ?? 01 C7 05 ?? ?? ?? ?? 01 00 00 00 83 7F 04 00 75 3D 8B 07 85 C0 74 0E 6A 20 50 E8 ?? ?? ?? ?? 89 07 5B 5E EB 0A 6A 20 E8 }
	condition:
		$pattern
}

rule _vfwprintf_internal_cae730ee26795fe33335a89aa27cd271 {
	meta:
		aliases = "_vfwprintf_internal"
		size = "1835"
		objfiles = "_vfwprintf_internal@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 81 EC 7C 02 00 00 8B B4 24 94 02 00 00 31 C0 B9 2F 00 00 00 8D BC 24 10 01 00 00 F3 AB FF 8C 24 28 01 00 00 89 B4 24 10 01 00 00 C7 84 24 20 01 00 00 80 00 00 00 C7 84 24 70 02 00 00 00 00 00 00 89 B4 24 78 02 00 00 8D 84 24 70 02 00 00 50 6A FF 8D 84 24 80 02 00 00 50 6A 00 E8 ?? ?? ?? ?? 83 C4 10 40 75 0D C7 84 24 10 01 00 00 ?? ?? ?? ?? EB 7B BA 09 00 00 00 8D 84 24 38 01 00 00 C7 00 08 00 00 00 83 C0 04 4A 75 F4 89 F0 8D 9C 24 10 01 00 00 EB 2B 83 FA 25 75 23 83 C0 04 83 38 25 74 1B 89 84 24 10 01 00 00 53 E8 ?? ?? ?? ?? 59 85 C0 78 39 8B 84 24 10 01 00 00 EB 03 83 C0 04 8B 10 }
	condition:
		$pattern
}
...
rule __GI_getnameinfo_95d4679e246c7de2d597657e45fd4ac3 {
	meta:
...
rule _time_tzset_8e10941e98d4ae2a48908fe14bcb2c1b {
	meta:
		aliases = "_time_tzset"
		size = "945"
		objfiles = "tzset@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 81 EC 98 00 00 00 C7 84 24 94 00 00 00 00 00 00 00 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 84 24 8C 00 00 00 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C6 83 C4 14 85 C0 75 4A 6A 00 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C7 58 5A 85 FF 78 32 6A 44 8D 5C 24 14 53 57 E8 ?? ?? ?? ?? 83 C4 0C 85 C0 78 15 8D 04 03 39 D8 76 0E 80 78 FF 0A 75 08 C6 40 FF 00 89 DE EB 02 31 F6 57 E8 ?? ?? ?? ?? 5D 85 F6 74 06 8A 06 84 C0 75 2B C6 05 ?? ?? ?? ?? 00 31 C0 B9 0C 00 00 00 BF ?? ?? ?? ?? F3 AB 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 5E 5F E9 A4 02 00 00 3C 3A 0F 94 }
	condition:
		$pattern
}

rule __open_nameservers_7830816378e8a5ec61ff6b747c758e57 {
	meta:
		aliases = "__open_nameservers"
		size = "867"
		objfiles = "opennameservers@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 81 EC A4 01 00 00 83 3D ?? ?? ?? ?? 00 75 33 8D 44 24 08 50 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 5F 5D 85 C0 74 08 C7 44 24 48 00 00 00 00 8B 44 24 48 39 05 ?? ?? ?? ?? 74 0A A3 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 3D ?? ?? ?? ?? 00 0F 85 FA 02 00 00 C6 05 ?? ?? ?? ?? 05 C6 05 ?? ?? ?? ?? 03 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 44 24 0C 5B 5E 85 C0 0F 85 F7 01 00 00 E9 27 02 00 00 8D 84 24 08 01 00 00 E8 ?? ?? ?? ?? 89 C6 E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C5 68 ?? ?? ?? ?? 56 E8 ?? ?? ?? ?? 5A 59 89 C3 85 C0 0F 85 B8 00 00 00 89 E8 E8 ?? ?? ?? ?? C6 00 00 B9 07 00 00 00 8B 3C 24 89 D8 }
	condition:
		$pattern
}
...
rule _fpmaxtostr_2a5eadc3c377ba43e39607754249c979 {
	meta:
		aliases = "_fpmaxtostr"
		size = "1464"
		objfiles = "_fpmaxtostr@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 81 EC BC 00 00 00 DB AC 24 D4 00 00 00 8B 84 24 E0 00 00 00 8B 40 04 89 44 24 08 8B 94 24 E0 00 00 00 8B 2A 8A 4A 08 88 4C 24 12 C6 84 24 A6 00 00 00 65 88 C8 83 C8 20 3C 61 75 07 83 C1 06 88 4C 24 12 85 ED 79 05 BD 06 00 00 00 C6 84 24 B6 00 00 00 00 8B 94 24 E0 00 00 00 8B 42 0C A8 02 74 0A C6 84 24 B6 00 00 00 2B EB 0C A8 01 74 08 C6 84 24 B6 00 00 00 20 C6 84 24 B7 00 00 00 00 C7 44 24 3C 00 00 00 00 DD E0 DF E0 9E 7A 02 74 0C DD D8 C7 44 24 3C 08 00 00 00 EB 61 D9 EE D9 C9 DD E1 DF E0 9E 75 2A 7A 28 D9 E8 D8 F1 D9 CA C7 44 24 04 FF FF FF FF DD EA DF E0 DD D9 9E 0F 86 08 01 00 }
	condition:
		$pattern
}
...
rule __dns_lookup_9087ebc7ef8069ba27b2db136396565c {
	meta:
		aliases = "__dns_lookup"
		size = "1814"
		objfiles = "dnslookup@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 81 EC D0 01 00 00 8B 9C 24 E4 01 00 00 68 00 02 00 00 E8 ?? ?? ?? ?? 89 C5 53 E8 ?? ?? ?? ?? 89 44 24 14 59 5E 3D 7E 03 00 00 76 15 C7 44 24 1C FF FF FF FF C7 44 24 2C 00 00 00 00 E9 8D 06 00 00 8B 44 24 0C 05 82 00 00 00 50 E8 ?? ?? ?? ?? 89 44 24 30 5A 85 ED 0F 84 69 06 00 00 85 C0 0F 84 61 06 00 00 80 3B 00 0F 84 58 06 00 00 8B 44 24 0C 8A 44 18 FF 88 44 24 13 FF 74 24 0C 53 FF 74 24 34 E8 ?? ?? ?? ?? C7 44 24 28 FF FF FF FF C7 44 24 34 00 00 00 00 C7 44 24 3C FF FF FF FF C7 44 24 40 FF FF FF FF 83 C4 0C 8B 54 24 2C 8B 4C 24 0C 8D 54 0A 01 89 54 24 08 83 7C 24 1C FF 74 0A FF 74 }
	condition:
		$pattern
}

rule __GI_ttyname_r_7614351c121abd997336fac0e16a3838 {
	meta:
...
rule __ieee754_pow_211572ad36c9bb18e72d21d2bca46c61 {
	meta:
		aliases = "__GI_pow, pow, __ieee754_pow"
		size = "1896"
		objfiles = "e_pow@libm.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 81 EC D4 00 00 00 DD 84 24 E8 00 00 00 DD 5C 24 18 DD 84 24 F0 00 00 00 DD 5C 24 10 DD 44 24 18 DD 9C 24 98 00 00 00 8B AC 24 9C 00 00 00 8B 84 24 98 00 00 00 89 84 24 B8 00 00 00 DD 44 24 10 DD 9C 24 90 00 00 00 8B 84 24 94 00 00 00 89 84 24 A0 00 00 00 89 C7 8B 94 24 90 00 00 00 89 C3 81 E3 FF FF FF 7F 89 D9 09 D1 75 07 D9 E8 E9 DA 06 00 00 8B 84 24 9C 00 00 00 89 84 24 B4 00 00 00 89 EE 81 E6 FF FF FF 7F 81 FE 00 00 F0 7F 7F 22 0F 94 84 24 A7 00 00 00 75 0A 83 BC 24 B8 00 00 00 00 75 0E 81 FB 00 00 F0 7F 7F 06 75 11 85 D2 74 0D DD 44 24 10 DC 44 24 18 E9 8D 06 00 00 83 BC 24 B4 }
	condition:
		$pattern
}
...
rule vfscanf_4e5c57fd2debd4b6493df6d3145749e7 {
	meta:
		aliases = "__GI_vfscanf, vfscanf"
		size = "1578"
		objfiles = "vfscanf@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 81 EC E8 01 00 00 C7 84 24 2C 01 00 00 FF FF FF FF 31 C0 B9 09 00 00 00 8D BC 24 08 01 00 00 F3 AB 8B 84 24 FC 01 00 00 8B 40 34 89 44 24 04 85 C0 75 26 8B 9C 24 FC 01 00 00 83 C3 38 53 68 ?? ?? ?? ?? 8D 84 24 DC 01 00 00 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 FF B4 24 FC 01 00 00 8D 9C 24 98 01 00 00 53 E8 ?? ?? ?? ?? C7 84 24 C8 01 00 00 ?? ?? ?? ?? 8B 84 24 A4 01 00 00 8A 40 03 88 84 24 B4 01 00 00 8B 84 24 CC 01 00 00 89 84 24 D8 01 00 00 C7 84 24 44 01 00 00 00 00 00 00 8B BC 24 08 02 00 00 C6 44 24 0B 01 5D 58 89 DD E9 D0 04 00 00 C6 84 24 4C 01 00 00 01 C6 84 24 4D 01 }
	condition:
		$pattern
}

rule vfwscanf_ddfe785c3d138f245179b8f80bd3178a {
	meta:
		aliases = "__GI_vfwscanf, vfwscanf"
		size = "1654"
		objfiles = "vfwscanf@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 81 EC FC 00 00 00 C7 44 24 38 FF FF FF FF 31 C0 B9 09 00 00 00 8D 7C 24 14 F3 AB 8B 84 24 10 01 00 00 8B 40 34 89 44 24 0C 85 C0 75 26 8B 9C 24 10 01 00 00 83 C3 38 53 68 ?? ?? ?? ?? 8D 84 24 E8 00 00 00 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 FF B4 24 10 01 00 00 8D 9C 24 A4 00 00 00 53 E8 ?? ?? ?? ?? C7 84 24 D4 00 00 00 ?? ?? ?? ?? 8B 84 24 B0 00 00 00 8A 40 03 88 84 24 C0 00 00 00 C7 84 24 E4 00 00 00 ?? ?? ?? ?? C7 44 24 50 00 00 00 00 8B BC 24 1C 01 00 00 C6 44 24 13 01 58 5A E9 08 05 00 00 C6 44 24 58 01 C6 44 24 59 00 80 A4 24 B9 00 00 00 01 C7 44 24 54 FF FF FF 7F C7 }
	condition:
		$pattern
}
...
rule __GI_inet_aton_6123089d71978e99f3fd102f30e385cd {
	meta:
...
rule _dl_linux_resolver_871dbf791585b6134e437cfad2fa6873 {
	meta:
...
rule __error_at_line_ed64e0cb05918d42eed96c252ab15275 {
	meta:
...
rule __psfs_parse_spec_c51bd9a18ccf73c533efb6d26903cb5d {
	meta:
		aliases = "__psfs_parse_spec"
		size = "440"
		objfiles = "__psfs_parse_spec@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 04 8B 74 24 18 8B 46 30 89 04 24 8A 00 83 E8 30 31 ED BF 01 00 00 00 3C 09 77 4B 81 FD CB CC CC 0C 7F 11 8B 46 30 0F B6 08 6B D5 0A 8D 6C 11 D0 40 89 46 30 8B 5E 30 8A 0B 8D 41 D0 3C 09 76 DB 80 F9 24 74 19 83 7E 24 00 0F 89 4F 01 00 00 89 6E 40 C7 46 24 FE FF FF FF E9 99 00 00 00 8D 43 01 89 46 30 31 FF BB ?? ?? ?? ?? BA 10 00 00 00 8B 4E 30 8A 03 3A 01 75 0B 8D 41 01 89 46 30 08 56 45 EB E2 43 80 3B 00 74 04 01 D2 EB E2 F6 46 45 10 74 08 C6 46 44 00 31 D2 EB 4F 89 F8 84 C0 74 13 83 7E 24 00 0F 89 F2 00 00 00 C7 46 24 FE FF FF FF EB E3 83 7E 24 FE 0F 84 DF 00 00 00 8D 45 FF }
	condition:
		$pattern
}
...
rule __GI_xdr_union_8ec4f68fd8621f2e9995a627566a440a {
	meta:
...
rule _dl_do_reloc_7bae9c1b0abde0bc6ea9bb788747dd88 {
	meta:
		aliases = "_dl_do_reloc"
		size = "225"
		objfiles = "libdl@libdl.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 04 8B 7C 24 18 8B 44 24 20 8B 17 89 14 24 8B 28 8B 40 04 0F B6 D8 C1 E8 08 89 C6 C1 E6 04 03 74 24 24 8B 0E 85 C0 74 68 83 FB 07 74 19 83 FB 23 74 14 83 FB 24 74 0F 83 FB 25 74 0A 31 D2 83 FB 0E 0F 94 C2 EB 05 BA 01 00 00 00 31 C0 83 FB 05 0F 94 C0 01 C0 09 D0 50 57 FF 74 24 24 8B 44 24 34 01 C8 50 E8 ?? ?? ?? ?? 89 C1 83 C4 10 85 C0 75 1A 8A 56 0C 89 D0 83 E0 0F 83 F8 06 74 0D C0 EA 04 B8 01 00 00 00 80 FA 02 75 49 89 C8 EB 03 8B 46 04 8B 0C 24 8D 54 0D 00 83 FB 08 77 07 FF 24 9D ?? ?? ?? ?? 83 C8 FF EB 2A 29 D0 EB 06 89 02 EB 20 8B 07 01 02 EB 1A 85 C0 74 16 8B 5E 08 8D 4A }
	condition:
		$pattern
}

rule __getdents_8eeb51e2355992606b2aefbebe100093 {
	meta:
		aliases = "__getdents"
		size = "130"
		objfiles = "getdents@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 04 8B 7C 24 18 8B 4C 24 1C 8B 54 24 20 53 89 FB B8 8D 00 00 00 CD 80 5B 89 C3 3D 00 F0 FF FF 76 0E E8 ?? ?? ?? ?? F7 DB 89 18 83 CE FF EB 45 89 C6 83 F8 FF 74 3E 89 CF 8D 2C 01 EB 33 0F B7 47 08 8A 44 07 FF 88 44 24 03 8D 5F 0A 53 E8 ?? ?? ?? ?? 40 89 04 24 53 8D 47 0B 50 E8 ?? ?? ?? ?? 8A 44 24 0F 88 47 0A 0F B7 47 08 01 C7 83 C4 0C 39 EF 72 C9 89 F0 5A 5B 5E 5F 5D C3 }
	condition:
		$pattern
}
...
rule __GI_strcasestr_e423ab6305e7e0b45dda86e461d450c3 {
	meta:
...
rule __GI_svc_register_cd5946b2894447ad99a25d357838654c {
	meta:
...
rule __GI_svc_getreqset_dda7e3b21ba955277fe74d029e7be01d {
	meta:
...
rule do_dlclose_1af0e247718da0b97e764a7f3548e05e {
	meta:
		aliases = "do_dlclose"
		size = "520"
		objfiles = "libdl@libdl.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 08 89 C5 89 14 24 3B 05 ?? ?? ?? ?? 0F 84 E7 01 00 00 A1 ?? ?? ?? ?? 31 D2 EB 09 39 E8 74 1A 89 C2 8B 40 04 85 C0 75 F3 C7 05 ?? ?? ?? ?? 0A 00 00 00 B0 01 E9 C2 01 00 00 85 D2 8B 45 04 74 05 89 42 04 EB 05 A3 ?? ?? ?? ?? 8B 55 00 8B 42 20 C7 44 24 04 00 00 00 00 66 83 F8 01 0F 84 53 01 00 00 48 66 89 42 20 55 E8 ?? ?? ?? ?? 31 C0 5B E9 86 01 00 00 8B 45 08 8B 54 24 04 8B 3C 90 8B 47 20 48 66 89 47 20 66 85 C0 0F 85 21 01 00 00 83 7F 74 00 75 09 83 BF A8 00 00 00 00 74 27 83 3C 24 00 74 21 66 8B 47 22 A8 08 75 19 83 C8 08 66 89 47 22 57 E8 ?? ?? ?? ?? 8B 47 74 59 85 C0 74 04 }
	condition:
		$pattern
}

rule __GI_strverscmp_4b10e48ee0d66d1ebe7e1b45a7805edc {
	meta:
...
rule __GI_inet_network_4a3e0802d21cba540a87bee663f3c332 {
	meta:
...
rule fgetwc_unlocked_80ada2a6a62c22e17ec023ecf319c7dd {
	meta:
...
rule __GI_setstate_r_5b60b5b77a6d54d2705c94253dc0e45c {
	meta:
...
rule __GI___ns_name_unpack_62c80aa01348ab92847092bb08fec35e {
	meta:
...
rule __decode_dotted_bf5c611fe74ed2d648e24b292af57226 {
	meta:
		aliases = "__decode_dotted"
		size = "262"
		objfiles = "decoded@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 0C 8B 54 24 24 83 7C 24 20 00 0F 84 E5 00 00 00 C6 44 24 07 01 31 ED C7 44 24 08 00 00 00 00 EB 04 89 7C 24 08 3B 54 24 28 0F 8D C6 00 00 00 8B 4C 24 20 8A 04 11 84 C0 0F 84 AB 00 00 00 80 7C 24 07 01 83 DD FF 0F B6 F0 8D 4A 01 89 F0 25 C0 00 00 00 3D C0 00 00 00 75 2F 3B 4C 24 28 0F 8D 91 00 00 00 80 7C 24 07 01 83 DD FF 89 F2 83 E2 3F C1 E2 08 8B 5C 24 20 0F B6 04 0B 09 C2 8B 7C 24 08 C6 44 24 07 00 EB 98 8B 44 24 08 01 F0 89 04 24 89 C7 47 3B 7C 24 30 73 5A 8D 1C 0E 8D 43 01 3B 44 24 28 73 4E 56 8B 44 24 24 01 C8 50 8B 44 24 34 03 44 24 10 50 E8 ?? ?? ?? ?? 89 DA 83 C4 0C }
	condition:
		$pattern
}
...
rule __read_etc_hosts_r_2f8e4ea9d154f17e99a9b05ce498e04b {
	meta:
		aliases = "__read_etc_hosts_r"
		size = "542"
		objfiles = "read_etc_hosts_r@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 54 24 38 8B 4C 24 3C 8B 74 24 44 C7 06 FF FF FF FF 89 D0 F7 D8 83 E0 03 8D 1C 02 29 C1 89 C8 83 E8 20 89 44 24 08 0F 88 DB 01 00 00 8D 53 20 89 54 24 0C 83 7C 24 30 01 75 11 C7 04 24 00 00 00 00 C7 44 24 04 00 00 00 00 EB 55 83 E9 38 89 4C 24 08 83 F9 4F 0F 8E AC 01 00 00 E8 ?? ?? ?? ?? 89 44 24 24 85 C0 75 16 8B 44 24 40 C7 00 00 00 00 00 E8 ?? ?? ?? ?? 8B 28 E9 8E 01 00 00 8B 54 24 0C 89 14 24 8D 43 28 89 44 24 04 8D 53 38 89 54 24 0C 89 43 20 8B 04 24 C7 40 04 00 00 00 00 C7 06 01 00 00 00 BD 01 00 00 00 E9 28 01 00 00 6A 23 FF 74 24 10 E8 ?? ?? ?? ?? 5A 59 C6 00 00 }
	condition:
		$pattern
}

rule getprotobyname_r_f052a7a79f1a57e84fd13df66f67c559 {
	meta:
		aliases = "__GI_getprotobyname_r, getprotobyname_r"
		size = "182"
		objfiles = "getproto@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 6C 24 24 8B 74 24 28 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 0F BE 05 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 83 C4 14 EB 29 55 FF 36 E8 ?? ?? ?? ?? 59 5B 85 C0 74 36 8B 5E 04 EB 10 55 50 E8 ?? ?? ?? ?? 59 5A 85 C0 74 24 83 C3 04 8B 03 85 C0 75 EA FF 74 24 34 FF 74 24 34 FF 74 24 34 56 E8 ?? ?? ?? ?? 89 C7 83 C4 10 85 C0 74 BC 80 3D ?? ?? ?? ?? 00 75 05 E8 ?? ?? ?? ?? 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 8B 44 24 3C 83 38 00 0F 94 C0 0F B6 C0 F7 D8 21 F8 83 C4 18 5B 5E 5F 5D C3 }
	condition:
		$pattern
}

rule __GI_getprotoent_r_7cec24832cb8a92046e95af2bee6fced {
	meta:
		aliases = "getprotoent_r, __GI_getprotoent_r"
		size = "415"
		objfiles = "getproto@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 6C 24 24 8B 7C 24 28 8B 5C 24 2C 8B 44 24 30 C7 00 00 00 00 00 81 FB 8B 00 00 00 77 15 E8 ?? ?? ?? ?? C7 00 22 00 00 00 BB 22 00 00 00 E9 5B 01 00 00 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 8D B7 8C 00 00 00 8D 83 74 FF FF FF 83 C4 10 3D 00 10 00 00 77 15 E8 ?? ?? ?? ?? C7 00 22 00 00 00 BB 22 00 00 00 E9 04 01 00 00 83 3D ?? ?? ?? ?? 00 75 26 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? A3 ?? ?? ?? ?? 5B 5A 85 C0 75 0C E8 ?? ?? ?? ?? 8B 18 E9 D5 00 00 00 FF 35 ?? ?? ?? ?? 68 00 10 00 00 56 E8 ?? ?? ?? ?? 89 C3 83 C4 0C }
	condition:
		$pattern
}

rule __GI_getservbyname_r_e1bc00674f859a06e76116767eaea206 {
	meta:
		aliases = "getservbyname_r, __GI_getservbyname_r"
		size = "209"
		objfiles = "getservice@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 6C 24 28 8B 74 24 2C 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 0F BE 05 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 83 C4 14 EB 44 FF 36 FF 74 24 28 E8 ?? ?? ?? ?? 5B 5A 85 C0 74 20 8B 5E 04 EB 13 50 FF 74 24 28 E8 ?? ?? ?? ?? 5A 59 85 C0 74 0B 83 C3 04 8B 03 85 C0 75 E7 EB 13 85 ED 74 2A 55 FF 76 0C E8 ?? ?? ?? ?? 59 5B 85 C0 74 1B FF 74 24 38 FF 74 24 38 FF 74 24 38 56 E8 ?? ?? ?? ?? 89 C7 83 C4 10 85 C0 74 A1 80 3D ?? ?? ?? ?? 00 75 05 E8 ?? ?? ?? ?? 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 8B 44 24 40 83 38 00 0F 94 C0 0F B6 C0 F7 D8 21 }
	condition:
		$pattern
}

rule fwide_a303457c78d95864edb1cabd0a94543d {
	meta:
		aliases = "fwide"
		size = "127"
		objfiles = "fwide@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 74 24 24 8B 6C 24 28 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 85 ED 74 1D 8B 16 F7 C2 80 08 00 00 75 13 B8 00 08 00 00 85 ED 7F 05 B8 80 00 00 00 09 D0 66 89 06 0F B7 1E 85 FF 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 25 00 08 00 00 81 E3 80 00 00 00 29 D8 83 C4 10 5B 5E 5F 5D C3 }
	condition:
		$pattern
}

rule __GI_readdir_r_327ce9f361062c1cb60313159f86acd4 {
	meta:
		aliases = "readdir_r, __GI_readdir_r"
		size = "190"
		objfiles = "readdir_r@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 74 24 24 8B 6C 24 2C 8D 5E 18 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 31 FF 83 C4 10 8B 46 08 3B 46 04 77 35 FF 76 14 FF 76 0C FF 36 E8 ?? ?? ?? ?? 83 C4 0C 83 F8 00 7F 16 C7 45 00 00 00 00 00 75 04 31 DB EB 46 E8 ?? ?? ?? ?? 8B 18 EB 3D 89 46 08 C7 46 04 00 00 00 00 8B 46 04 89 C7 03 7E 0C 0F B7 57 08 01 C2 89 56 04 8B 47 04 89 46 10 83 3F 00 74 A7 0F B7 47 08 50 57 FF 74 24 30 E8 ?? ?? ?? ?? 89 45 00 31 DB 83 C4 0C 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 31 C0 85 FF 0F 94 C0 F7 D8 21 C3 89 D8 83 C4 18 5B 5E 5F 5D C3 }
	condition:
		$pattern
}

rule __GI_readdir64_r_6e1612c9b595f57fa660aa0c258dcdf3 {
	meta:
		aliases = "readdir64_r, __GI_readdir64_r"
		size = "192"
		objfiles = "readdir64_r@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 74 24 24 8B 6C 24 2C 8D 5E 18 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 31 FF 83 C4 10 8B 46 08 3B 46 04 77 35 FF 76 14 FF 76 0C FF 36 E8 ?? ?? ?? ?? 83 C4 0C 83 F8 00 7F 16 C7 45 00 00 00 00 00 75 04 31 DB EB 48 E8 ?? ?? ?? ?? 8B 18 EB 3F 89 46 08 C7 46 04 00 00 00 00 8B 56 04 89 D7 03 7E 0C 0F B7 47 10 01 D0 89 46 04 8B 47 08 89 46 10 8B 07 0B 47 04 74 A5 0F B7 47 10 50 57 FF 74 24 30 E8 ?? ?? ?? ?? 89 45 00 31 DB 83 C4 0C 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 31 C0 85 FF 0F 94 C0 F7 D8 21 C3 89 D8 83 C4 18 5B 5E 5F 5D C3 }
	condition:
		$pattern
}

rule putgrent_1e13ce581b0a136f102820b4b6bcd72f {
	meta:
		aliases = "putgrent"
		size = "191"
		objfiles = "putgrent@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 74 24 24 8B 7C 24 28 85 F6 74 04 85 FF 75 13 E8 ?? ?? ?? ?? C7 00 16 00 00 00 83 CB FF E9 8B 00 00 00 8B 6F 34 85 ED 75 1C 8D 5F 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 FF 76 08 FF 76 04 FF 36 68 ?? ?? ?? ?? 57 E8 ?? ?? ?? ?? 83 C4 14 85 C0 78 39 8B 5E 0C BA ?? ?? ?? ?? 8B 03 85 C0 75 12 57 6A 0A E8 ?? ?? ?? ?? 59 5B 31 DB 85 C0 79 1E EB 19 50 52 57 E8 ?? ?? ?? ?? 83 C4 0C 85 C0 78 0A 83 C3 04 BA ?? ?? ?? ?? EB CF 83 CB FF 85 ED 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 C4 10 5B 5E 5F 5D C3 }
	condition:
		$pattern
}

rule fgetpos_42214421f580a26c37cf428fa00fff62 {
	meta:
		aliases = "fgetpos"
		size = "115"
		objfiles = "fgetpos@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 74 24 24 8B 7C 24 28 8B 6E 34 85 ED 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 56 E8 ?? ?? ?? ?? 89 07 59 83 CB FF 85 C0 78 15 8B 46 2C 89 47 04 8B 46 30 89 47 08 0F B6 46 02 89 47 0C 31 DB 85 ED 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 C4 10 5B 5E 5F 5D C3 }
	condition:
		$pattern
}

rule fgetpos64_803b366bc07bcb77b0535ac6d41d3549 {
	meta:
		aliases = "fgetpos64"
		size = "118"
		objfiles = "fgetpos64@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 74 24 24 8B 7C 24 28 8B 6E 34 85 ED 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 56 E8 ?? ?? ?? ?? 89 07 89 57 04 59 83 CB FF 85 D2 78 15 8B 46 2C 89 47 08 8B 46 30 89 47 0C 0F B6 46 02 89 47 10 31 DB 85 ED 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 C4 10 5B 5E 5F 5D C3 }
	condition:
		$pattern
}

rule fsetpos_8f981292ae4af6f520f9fbf5ceb7c4d3 {
	meta:
		aliases = "fsetpos"
		size = "115"
		objfiles = "fsetpos@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 74 24 24 8B 7C 24 28 8B 6E 34 85 ED 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 6A 00 FF 37 56 E8 ?? ?? ?? ?? 89 C3 83 C4 0C 85 C0 75 12 8B 47 04 89 46 2C 8B 47 08 89 46 30 8B 47 0C 88 46 02 85 ED 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 C4 10 5B 5E 5F 5D C3 }
	condition:
		$pattern
}

rule fsetpos64_bc19b1d8c509b15188201dc34a6863e7 {
	meta:
		aliases = "fsetpos64"
		size = "118"
		objfiles = "fsetpos64@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 74 24 24 8B 7C 24 28 8B 6E 34 85 ED 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 6A 00 FF 77 04 FF 37 56 E8 ?? ?? ?? ?? 89 C3 83 C4 10 85 C0 75 12 8B 47 08 89 46 2C 8B 47 0C 89 46 30 8B 47 10 88 46 02 85 ED 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 C4 10 5B 5E 5F 5D C3 }
	condition:
		$pattern
}

rule getprotobynumber_r_52bda665fc9432d7cbf64303c330f088 {
	meta:
		aliases = "__GI_getprotobynumber_r, getprotobynumber_r"
		size = "143"
		objfiles = "getproto@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 74 24 28 8B 6C 24 30 8B 7C 24 34 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 0F BE 05 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 83 C4 14 EB 09 8B 44 24 24 39 46 08 74 15 57 55 FF 74 24 34 56 E8 ?? ?? ?? ?? 89 C3 83 C4 10 85 C0 74 E2 80 3D ?? ?? ?? ?? 00 75 05 E8 ?? ?? ?? ?? 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 31 C0 83 3F 00 0F 94 C0 F7 D8 21 D8 83 C4 18 5B 5E 5F 5D C3 }
	condition:
		$pattern
}

rule _dl_load_shared_library_d485ffa8278e18b0c56d996141a3eacc {
	meta:
		aliases = "_dl_load_shared_library"
		size = "474"
		objfiles = "libdl@libdl.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 74 24 2C 8B 5C 24 30 C7 05 ?? ?? ?? ?? 00 00 00 00 8D 43 FF 40 80 38 00 75 FA 29 D8 3D 00 04 00 00 0F 87 87 01 00 00 8D 43 FF 31 C9 EB 07 80 FA 2F 75 02 89 C1 40 8A 10 84 D2 75 F2 89 DF 85 C9 74 03 8D 79 01 39 DF 74 19 53 FF 74 24 2C FF 74 24 2C E8 ?? ?? ?? ?? 83 C4 0C 85 C0 0F 85 68 01 00 00 85 F6 74 22 8B 4E 7C 85 C9 74 1B 03 4E 54 FF 74 24 28 8B 54 24 28 89 F8 E8 ?? ?? ?? ?? 5D 85 C0 0F 85 42 01 00 00 8B 0D ?? ?? ?? ?? 85 C9 74 18 FF 74 24 28 8B 54 24 28 89 F8 E8 ?? ?? ?? ?? 5B 85 C0 0F 85 20 01 00 00 85 F6 74 25 8B 8E B4 00 00 00 85 C9 74 1B 03 4E 54 FF 74 24 28 8B }
	condition:
		$pattern
}

rule __GI_rresvport_861bc12032b8c2ea393c0da4d66fa7f2 {
	meta:
...
rule __GI_unsetenv_639884a97fb7d3b9fcfa23ff02b3a088 {
	meta:
...
rule __GI_ungetwc_a6662f968dca82b84ffd574c3410bfb0 {
	meta:
		aliases = "ungetwc, __GI_ungetwc"
		size = "161"
		objfiles = "ungetwc@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 7C 24 24 8B 74 24 28 8B 6E 34 85 ED 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 0F B7 06 25 03 08 00 00 3D 00 08 00 00 77 11 68 00 08 00 00 56 E8 ?? ?? ?? ?? 59 5B 85 C0 75 30 0F B7 06 A8 02 74 0A A8 01 75 25 83 7E 28 00 75 1F 83 FF FF 74 1A C7 46 28 01 00 00 00 8B 06 40 66 89 06 83 E0 01 89 7C 86 24 66 83 26 FB EB 03 83 CF FF 85 ED 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 F8 83 C4 10 5B 5E 5F 5D C3 }
	condition:
		$pattern
}

rule ungetc_74a22af303c1fe58706f08b2830ec2c8 {
	meta:
		aliases = "__GI_ungetc, ungetc"
		size = "198"
		objfiles = "ungetc@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 7C 24 24 8B 74 24 28 8B 6E 34 85 ED 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 8B 46 10 3B 46 18 73 17 83 FF FF 74 12 3B 46 08 76 0D 89 FA 38 50 FF 75 06 48 89 46 10 EB 50 0F B7 06 25 83 00 00 00 3D 80 00 00 00 77 11 68 80 00 00 00 56 E8 ?? ?? ?? ?? 59 5B 85 C0 75 36 0F B7 06 A8 02 74 0A A8 01 75 2B 83 7E 28 00 75 25 83 FF FF 74 20 8B 46 08 89 46 18 C7 46 28 01 00 00 00 8B 06 40 66 89 06 83 E0 01 89 7C 86 24 66 83 26 FB EB 03 83 CF FF 85 ED 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 F8 83 C4 10 5B 5E 5F 5D C3 }
	condition:
		$pattern
}

rule putspent_10660986db3e9c7952eb5f8d8746588f {
	meta:
		aliases = "putspent"
		size = "207"
		objfiles = "putspent@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 7C 24 24 8B 74 24 28 8B 6E 34 85 ED 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 8B 47 04 85 C0 75 05 B8 ?? ?? ?? ?? 50 FF 37 68 ?? ?? ?? ?? 56 E8 ?? ?? ?? ?? 83 C4 10 31 DB 85 C0 79 2B EB 59 0F B6 83 ?? ?? ?? ?? 8B 04 07 BA ?? ?? ?? ?? 83 F8 FF 74 05 BA ?? ?? ?? ?? 50 52 56 E8 ?? ?? ?? ?? 83 C4 0C 85 C0 78 31 43 83 FB 05 76 D2 8B 47 20 83 F8 FF 74 13 50 68 ?? ?? ?? ?? 56 E8 ?? ?? ?? ?? 83 C4 0C 85 C0 78 10 56 6A 0A E8 ?? ?? ?? ?? 59 5B 31 DB 85 C0 7F 03 83 CB FF 85 ED 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 }
	condition:
		$pattern
}

rule getservbyport_r_246f8d86306f15620e3aab50feb1be77 {
	meta:
		aliases = "__GI_getservbyport_r, getservbyport_r"
		size = "166"
		objfiles = "getservice@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 10 8B 7C 24 28 8B 5C 24 2C 8B 6C 24 38 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 0F BE 05 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 83 C4 14 EB 1C 8B 44 24 24 39 43 08 75 13 85 FF 74 27 57 FF 73 0C E8 ?? ?? ?? ?? 59 5A 85 C0 74 18 55 FF 74 24 38 FF 74 24 38 53 E8 ?? ?? ?? ?? 89 C6 83 C4 10 85 C0 74 CC 80 3D ?? ?? ?? ?? 00 75 05 E8 ?? ?? ?? ?? 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 31 C0 83 7D 00 00 0F 94 C0 F7 D8 21 F0 83 C4 18 5B 5E 5F 5D C3 }
	condition:
		$pattern
}
...
rule __GI_lround_3095c0570d8623de324693f16e6ae11b {
	meta:
...
rule fcloseall_92246116a2f3ad93166185bcf4097765 {
	meta:
		aliases = "fcloseall"
		size = "219"
		objfiles = "fcloseall@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 14 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 5C 24 0C 53 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? A1 ?? ?? ?? ?? 40 A3 ?? ?? ?? ?? 6A 01 53 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 24 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B 35 ?? ?? ?? ?? 6A 01 53 E8 ?? ?? ?? ?? C7 44 24 0C 00 00 00 00 83 C4 0C EB 57 8B 6E 20 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 0C 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 0F B7 06 83 E0 30 83 F8 30 74 12 56 E8 ?? ?? ?? ?? 59 85 C0 74 07 C7 04 24 FF FF FF FF 85 FF 75 0E 6A 01 8D 44 24 08 50 E8 ?? ?? ?? ?? 58 5A 89 EE 85 }
	condition:
		$pattern
}

rule __GI_fflush_unlocked_f4729ddad3b99376cce0da6fe21e2646 {
	meta:
		aliases = "fflush_unlocked, __GI_fflush_unlocked"
		size = "300"
		objfiles = "fflush_unlocked@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 14 8B 5C 24 28 66 C7 44 24 02 00 00 81 FB ?? ?? ?? ?? 74 0F 66 C7 44 24 02 00 01 85 DB 0F 85 D8 00 00 00 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 5C 24 0C 53 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? A1 ?? ?? ?? ?? 40 A3 ?? ?? ?? ?? 6A 01 53 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 24 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B 35 ?? ?? ?? ?? 6A 01 53 E8 ?? ?? ?? ?? 31 FF 83 C4 0C 89 DD EB 68 F6 06 40 74 60 83 3D ?? ?? ?? ?? 02 74 18 8D 5E 38 53 68 ?? ?? ?? ?? 55 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 66 8B 44 24 02 0B 06 66 35 40 01 A9 40 03 00 00 75 1A 56 E8 }
	condition:
		$pattern
}
...
rule getdelim_ecf3f953469f08dfbbf5406ac8f97cd0 {
	meta:
		aliases = "__GI_getdelim, getdelim"
		size = "235"
		objfiles = "getdelim@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 14 8B 6C 24 2C 8B 7C 24 34 83 7C 24 28 00 74 08 85 ED 74 04 85 FF 75 13 E8 ?? ?? ?? ?? C7 00 16 00 00 00 83 CF FF E9 B0 00 00 00 8B 47 34 89 04 24 85 C0 75 1C 8D 5F 38 53 68 ?? ?? ?? ?? 8D 44 24 0C 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 8B 44 24 28 8B 18 85 DB 75 07 C7 45 00 00 00 00 00 BE 01 00 00 00 8B 45 00 39 C6 72 21 83 C0 40 50 53 E8 ?? ?? ?? ?? 5B 5A 85 C0 75 05 83 CF FF EB 44 89 C3 83 45 00 40 8B 44 24 28 89 18 8B 47 10 3B 47 18 73 09 0F B6 10 40 89 47 10 EB 0E 57 E8 ?? ?? ?? ?? 89 C2 59 83 F8 FF 74 0B 46 88 54 1E FE 3B 54 24 30 75 AE 89 F7 83 EF 02 78 07 8D 7E }
	condition:
		$pattern
}
...
rule __GI_getservent_r_7d86d3f40fe6b3d340dc350d970a04a5 {
	meta:
		aliases = "getservent_r, __GI_getservent_r"
		size = "469"
		objfiles = "getservice@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 14 8B 7C 24 28 8B 6C 24 2C 8B 5C 24 30 8B 44 24 34 C7 00 00 00 00 00 81 FB 8B 00 00 00 77 15 E8 ?? ?? ?? ?? C7 00 22 00 00 00 BB 22 00 00 00 E9 91 01 00 00 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 0C 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 8D 85 8C 00 00 00 89 44 24 10 8D 83 74 FF FF FF 83 C4 10 3D 00 10 00 00 77 15 E8 ?? ?? ?? ?? C7 00 22 00 00 00 BB 22 00 00 00 E9 36 01 00 00 83 3D ?? ?? ?? ?? 00 75 2F 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? A3 ?? ?? ?? ?? 5B 5E 85 C0 75 15 E8 ?? ?? ?? ?? C7 00 05 00 00 00 BB 05 00 00 00 E9 FE 00 00 00 FF 35 ?? ?? ?? ?? 68 00 10 }
	condition:
		$pattern
}

rule __GI_setvbuf_70d7d584bd35e7a46b4f1ae62211121d {
	meta:
		aliases = "setvbuf, __GI_setvbuf"
		size = "241"
		objfiles = "setvbuf@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 14 8B 7C 24 28 8B 74 24 2C 8B 6C 24 34 8B 47 34 89 04 24 85 C0 75 1C 8D 5F 38 53 68 ?? ?? ?? ?? 8D 44 24 0C 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 83 7C 24 30 02 76 13 E8 ?? ?? ?? ?? C7 00 16 00 00 00 83 CB FF E9 80 00 00 00 8B 17 83 CB FF F7 C2 CF 08 00 00 75 73 8B 44 24 30 C1 E0 08 80 E6 FC 09 C2 66 89 17 83 7C 24 30 02 74 04 85 ED 75 08 31 F6 31 ED 31 DB EB 21 31 DB 85 F6 75 1B 8B 47 0C 2B 47 08 39 E8 74 3F 55 E8 ?? ?? ?? ?? 5E 85 C0 74 34 89 C6 66 BB 00 40 8B 07 F6 C4 40 74 0F 80 E4 BF 66 89 07 FF 77 08 E8 ?? ?? ?? ?? 59 66 09 1F 89 77 08 8D 04 2E 89 47 0C 89 77 10 }
	condition:
		$pattern
}
...
rule __GI_fseeko64_03a38c041985e1238da78fa848dbf850 {
	meta:
		aliases = "fseeko64, __GI_fseeko64"
		size = "212"
		objfiles = "fseeko64@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 18 8B 74 24 2C 8B 7C 24 38 8B 44 24 30 8B 54 24 34 89 44 24 10 89 54 24 14 83 FF 02 76 13 E8 ?? ?? ?? ?? C7 00 16 00 00 00 83 CB FF E9 93 00 00 00 8B 6E 34 85 ED 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 F6 06 40 74 0B 56 E8 ?? ?? ?? ?? 5A 85 C0 75 4B 83 FF 01 75 11 8D 44 24 10 50 56 E8 ?? ?? ?? ?? 59 5B 85 C0 78 35 57 8D 44 24 14 50 56 E8 ?? ?? ?? ?? 83 C4 0C 85 C0 78 22 66 83 26 B8 8B 46 08 89 46 10 89 46 14 89 46 18 89 46 1C C7 46 2C 00 00 00 00 C6 46 02 00 31 DB EB 03 83 CB FF 85 ED 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? }
	condition:
		$pattern
}

rule __pgsreader_2089824d7e92cbb4afd4c24f363f1251 {
	meta:
		aliases = "__pgsreader"
		size = "265"
		objfiles = "__pgsreader@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 18 8B 74 24 34 8B 7C 24 38 8B 6C 24 3C 81 FF FF 00 00 00 77 15 E8 ?? ?? ?? ?? C7 00 22 00 00 00 BB 22 00 00 00 E9 CF 00 00 00 8B 45 34 89 44 24 04 85 C0 74 0A 31 DB 8D 14 3E 89 14 24 EB 1E 8D 5D 38 53 68 ?? ?? ?? ?? 8D 44 24 10 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 EB D8 55 57 56 E8 ?? ?? ?? ?? 83 C4 0C 85 C0 75 14 0F B7 45 00 83 E0 04 83 F8 01 19 DB 83 E3 20 83 C3 02 EB 64 56 E8 ?? ?? ?? ?? 59 8D 54 06 FF 80 3A 0A 75 05 C6 02 00 EB 08 40 39 F8 75 03 43 EB C0 85 DB 74 03 4B EB B9 8A 06 84 C0 74 B3 3C 23 74 AF 0F B6 D0 A1 ?? ?? ?? ?? F6 04 50 20 75 A1 81 7C 24 2C ?? ?? }
	condition:
		$pattern
}
...
rule __GI___res_init_81ddffd74a8687213e3ff1b85e76eaf0 {
	meta:
...
rule __GI_hsearch_r_108ac4ca44692572913c62f2c107be70 {
	meta:
		aliases = "hsearch_r, __GI_hsearch_r"
		size = "368"
		objfiles = "hsearch_r@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 1C 8B 44 24 30 8B 54 24 34 89 54 24 14 89 44 24 10 50 E8 ?? ?? ?? ?? 5A 89 C1 EB 0F 89 C2 C1 E2 04 8B 5C 24 10 0F B6 04 0B 01 D0 49 83 F9 FF 75 EB 8B 54 24 40 8B 52 04 89 54 24 08 31 D2 F7 74 24 08 89 D6 85 D2 75 04 66 BE 01 00 8B 4C 24 40 8B 09 89 4C 24 0C 6B C6 0C 89 CB 01 C3 8B 03 85 C0 0F 84 8D 00 00 00 39 F0 75 1D FF 73 04 FF 74 24 14 E8 ?? ?? ?? ?? 5F 5D 85 C0 75 0B 8D 43 04 8B 5C 24 3C 89 03 EB 5C 8B 54 24 08 83 EA 02 89 F0 89 D1 31 D2 F7 F1 8D 6A 01 89 F3 8B 44 24 08 29 E8 89 44 24 04 39 EB 77 06 03 5C 24 04 EB 02 29 EB 39 F3 74 3E 6B C3 0C 8B 7C 24 0C 01 C7 8B 17 89 }
	condition:
		$pattern
}
...
rule __GI_svcudp_bufcreate_cbd28a2228f6098467680577ff1d57df {
	meta:
...
rule __GI_sqrt_8ab5a07ae7eb659de4346f0169c4a358 {
	meta:
...
rule inet_pton_571e5b31a3f5400c9facbdb4cf173ecf {
	meta:
		aliases = "__GI_inet_pton, inet_pton"
		size = "458"
		objfiles = "ntop@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 20 8B 44 24 34 8B 74 24 38 83 F8 02 74 0B 83 F8 0A 0F 85 97 01 00 00 EB 10 8B 54 24 3C 89 F0 E8 ?? ?? ?? ?? E9 93 01 00 00 31 C0 8D 54 24 10 89 D7 AB AB AB AB 80 3E 3A 75 0A 46 80 3E 3A 0F 85 66 01 00 00 89 74 24 0C 89 54 24 04 C7 44 24 08 00 00 00 00 E9 8B 00 00 00 46 0F B6 C3 83 C8 20 50 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 59 5F 85 C0 74 1B C1 E5 04 2D ?? ?? ?? ?? 09 C5 81 FD FF FF 00 00 0F 86 9A 00 00 00 E9 1E 01 00 00 80 FB 3A 75 5E 83 3C 24 00 75 19 83 7C 24 08 00 0F 85 08 01 00 00 8B 44 24 04 89 44 24 08 89 74 24 0C EB 78 80 3E 00 0F 84 F1 00 00 00 8B 54 24 04 83 C2 02 8D 4C }
	condition:
		$pattern
}
...
rule __GI_fclose_16ba0b1c77df02f5e46b53f941b60179 {
	meta:
		aliases = "fclose, __GI_fclose"
		size = "237"
		objfiles = "fclose@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 20 8B 74 24 34 8B 6E 34 85 ED 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 18 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 31 FF F6 06 40 74 09 56 E8 ?? ?? ?? ?? 89 C7 58 FF 76 04 E8 ?? ?? ?? ?? 5B 85 C0 79 03 83 CF FF C7 46 04 FF FF FF FF 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 5C 24 08 53 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? A1 ?? ?? ?? ?? 40 A3 ?? ?? ?? ?? 6A 01 53 E8 ?? ?? ?? ?? 8B 06 66 25 00 60 83 C8 30 66 89 06 83 C4 18 85 ED 75 0E 6A 01 8D 44 24 14 50 E8 ?? ?? ?? ?? 5A 59 F6 46 01 40 74 09 FF 76 08 E8 ?? ?? ?? ?? 58 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 68 ?? }
	condition:
		$pattern
}

rule freopen64_99fa562e7a235e54cbe89001335ecd85 {
	meta:
		aliases = "freopen64"
		size = "236"
		objfiles = "freopen64@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 20 8B 74 24 3C 8B 6E 34 85 ED 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 18 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 7C 24 08 57 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? A1 ?? ?? ?? ?? 40 A3 ?? ?? ?? ?? 6A 01 57 E8 ?? ?? ?? ?? 8B 1E 89 D8 80 E4 9F 66 89 06 83 E0 30 83 C4 18 83 F8 30 74 31 56 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 57 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? FF 0D ?? ?? ?? ?? 6A 01 57 E8 ?? ?? ?? ?? 83 C4 1C 6A FE 56 FF 74 24 40 FF 74 24 40 E8 ?? ?? ?? ?? 89 C7 83 C4 10 85 C0 75 05 66 C7 06 30 80 66 81 E3 00 60 }
	condition:
		$pattern
}

rule freopen_1032c1b38aa430e30d9e6e3518d992c6 {
	meta:
		aliases = "freopen"
		size = "236"
		objfiles = "freopen@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 20 8B 74 24 3C 8B 6E 34 85 ED 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 18 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 7C 24 08 57 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? A1 ?? ?? ?? ?? 40 A3 ?? ?? ?? ?? 6A 01 57 E8 ?? ?? ?? ?? 8B 1E 89 D8 80 E4 9F 66 89 06 83 E0 30 83 C4 18 83 F8 30 74 31 56 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 57 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? FF 0D ?? ?? ?? ?? 6A 01 57 E8 ?? ?? ?? ?? 83 C4 1C 6A FF 56 FF 74 24 40 FF 74 24 40 E8 ?? ?? ?? ?? 89 C7 83 C4 10 85 C0 75 05 66 C7 06 30 80 66 81 E3 00 60 }
	condition:
		$pattern
}
...
rule __GI_memcmp_a493628e52c272244c2cb8b5e245501b {
	meta:
...
rule _dl_dprintf_c570b9b6d9d4ab32ad2763785c544b44 {
	meta:
		aliases = "_dl_dprintf"
		size = "703"
		objfiles = "libdl@libdl.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 24 8B 5C 24 3C 85 DB 0F 84 A4 02 00 00 8B 0D ?? ?? ?? ?? C7 44 24 1C 00 00 00 00 31 C0 BA 03 00 00 00 BE 22 00 00 00 83 CF FF 53 89 C3 55 8B 6C 24 1C B8 C0 00 00 00 CD 80 5D 5B 3D 00 F0 FF FF 76 0C F7 D8 A3 ?? ?? ?? ?? 83 CD FF EB 07 89 C5 83 F8 FF 75 45 B9 ?? ?? ?? ?? BA 1D 00 00 00 8B 7C 24 38 53 89 FB B8 04 00 00 00 CD 80 5B 3D 00 F0 FF FF 76 07 F7 D8 A3 ?? ?? ?? ?? BF 14 00 00 00 53 89 FB B8 01 00 00 00 CD 80 5B 3D 00 F0 FF FF 76 07 F7 D8 A3 ?? ?? ?? ?? 8D 73 FF 89 F2 42 80 3A 00 75 FA 29 DA A1 ?? ?? ?? ?? 48 39 C2 72 45 B9 ?? ?? ?? ?? BA 0B 00 00 00 8B 7C 24 38 53 89 FB }
	condition:
		$pattern
}
...
rule __GI_nextafter_6a8187cabd4ac5229a4dd944cd260fad {
	meta:
...
rule __GI_lrint_0eb3a3397a28999c63cd454146012ac5 {
	meta:
...
rule sleep_d5763333fa9e50246bbd20cdb82e5b58 {
	meta:
		aliases = "__GI_sleep, sleep"
		size = "242"
		objfiles = "sleep@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 2C 8B 54 24 40 31 C0 85 D2 0F 84 D5 00 00 00 C7 44 24 1C 00 00 00 00 C7 44 24 20 00 00 00 00 C7 44 24 28 00 00 00 00 89 54 24 24 6A 11 8D 5C 24 20 53 E8 ?? ?? ?? ?? 8D 6C 24 1C 55 53 6A 00 E8 ?? ?? ?? ?? 6A 11 55 E8 ?? ?? ?? ?? 83 C4 1C 85 C0 75 6B C7 44 24 1C 00 00 00 00 C7 44 24 20 00 00 00 00 6A 11 53 E8 ?? ?? ?? ?? 8D 44 24 08 50 6A 00 6A 11 E8 ?? ?? ?? ?? 83 C4 14 83 3C 24 01 8D 5C 24 24 75 20 53 53 E8 ?? ?? ?? ?? 89 C7 E8 ?? ?? ?? ?? 89 C3 8B 30 6A 00 55 6A 02 E8 ?? ?? ?? ?? 89 33 EB 13 6A 00 55 6A 02 E8 ?? ?? ?? ?? 53 53 E8 ?? ?? ?? ?? 89 C7 83 C4 14 EB 0F 8D 44 24 24 }
	condition:
		$pattern
}

rule drem_9b7e963d6d8f0cd0936e1d4c9b1a8f40 {
	meta:
...
rule __GI_clnttcp_create_c8b367c2de8d8cd4921b46f3d9d833b9 {
	meta:
...
rule __GI_clntunix_create_86ddb257ec44567192d38173f0fa4643 {
	meta:
...
rule __ieee754_fmod_249a1f99426ab1ef9f347dab3822962f {
	meta:
...
rule malloc_eea1544d6b37429aa89699c6c9ab5c29 {
	meta:
		aliases = "malloc"
		size = "1930"
		objfiles = "malloc@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 34 8B 5C 24 48 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 2C 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 10 83 FB DF 76 12 E8 ?? ?? ?? ?? C7 00 0C 00 00 00 31 DB E9 3D 07 00 00 8D 43 0B C7 44 24 08 10 00 00 00 83 F8 0F 76 07 83 E0 F8 89 44 24 08 8B 1D ?? ?? ?? ?? F6 C3 01 75 18 85 DB 0F 85 5D 03 00 00 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 5D E9 4D 03 00 00 39 5C 24 08 77 1E 8B 4C 24 08 C1 E9 03 8B 14 8D ?? ?? ?? ?? 85 D2 74 0C 8B 42 08 89 04 8D ?? ?? ?? ?? EB 35 81 7C 24 08 FF 00 00 00 77 33 8B 6C 24 08 C1 ED 03 8D 0C ED ?? ?? ?? ?? 8B 51 0C 39 CA 0F 84 6D 01 00 00 8B 42 0C 8B }
	condition:
		$pattern
}

rule __GI_llrint_34cb6bc48114a314dfeb0d4aa8cdc576 {
	meta:
...
rule __xpg_strerror_r_f47c225702285995fff180a022a4c6cb {
	meta:
...
rule __ieee754_atan2_2d90a78732b78f753276456c8a03e6bd {
	meta:
...
rule __GI_gethostbyaddr_r_97a077f5eb793c656112222695364749 {
	meta:
		aliases = "gethostbyaddr_r, __GI_gethostbyaddr_r"
		size = "678"
		objfiles = "gethostbyaddr_r@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 40 8B 7C 24 64 8B 5C 24 68 8B 44 24 6C C7 00 00 00 00 00 83 7C 24 54 00 0F 84 75 02 00 00 83 7C 24 5C 02 74 0D 83 7C 24 5C 0A 0F 85 63 02 00 00 EB 07 83 7C 24 58 04 EB 05 83 7C 24 58 10 0F 85 4F 02 00 00 FF 74 24 70 FF 74 24 70 53 57 FF 74 24 70 FF 74 24 70 FF 74 24 70 FF 74 24 70 E8 ?? ?? ?? ?? 83 C4 20 85 C0 0F 84 2A 02 00 00 8B 4C 24 70 8B 11 83 FA 01 74 09 83 FA 04 0F 85 16 02 00 00 8B 44 24 70 C7 00 FF FF FF FF 89 FE F7 DE 83 E6 03 01 F7 89 7C 24 08 29 F3 83 C7 20 89 7C 24 0C 8B 54 24 08 83 C2 28 89 54 24 3C 83 EB 38 89 5C 24 04 B8 22 00 00 00 81 FB FF 00 00 00 0F 8E D3 }
	condition:
		$pattern
}
...
rule _dl_map_cache_ad2197ed68ba28bb397bf73976527459 {
	meta:
		aliases = "_dl_map_cache"
		size = "500"
		objfiles = "libdl@libdl.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 44 8B 15 ?? ?? ?? ?? 83 C8 FF 83 FA FF 0F 84 D3 01 00 00 85 D2 0F 85 C9 01 00 00 BE ?? ?? ?? ?? 89 E1 89 F0 53 89 C3 B8 6A 00 00 00 CD 80 5B 89 C3 3D 00 F0 FF FF 76 0D F7 DB 89 1D ?? ?? ?? ?? E9 90 01 00 00 85 C0 0F 85 88 01 00 00 B9 00 00 08 00 31 D2 53 89 F3 B8 05 00 00 00 CD 80 5B 89 C7 3D 00 F0 FF FF 76 0D F7 DF 89 3D ?? ?? ?? ?? E9 60 01 00 00 85 C0 0F 88 58 01 00 00 8B 4C 24 14 89 0D ?? ?? ?? ?? C7 44 24 40 00 00 00 00 BE 01 00 00 00 89 D8 89 F2 53 89 C3 55 8B 6C 24 40 B8 C0 00 00 00 CD 80 5D 5B 89 C1 89 C2 3D 00 F0 FF FF 76 0B F7 D9 89 0D ?? ?? ?? ?? 83 CA FF 89 15 ?? }
	condition:
		$pattern
}

rule timer_create_54e49d81b8b7c82ab7b7d20ac0e74a96 {
	meta:
		aliases = "timer_create"
		size = "148"
		objfiles = "timer_create@librt.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 44 8B 74 24 5C 85 F6 75 12 C7 44 24 08 00 00 00 00 C7 44 24 04 0E 00 00 00 89 E6 83 7E 08 02 74 62 6A 08 E8 ?? ?? ?? ?? 89 C5 59 85 C0 74 54 89 04 24 8D 54 24 40 8B 7C 24 58 89 F1 53 89 FB B8 03 01 00 00 CD 80 5B 89 C3 3D 00 F0 FF FF 76 0B E8 ?? ?? ?? ?? F7 DB 89 18 EB 1C 83 F8 FF 74 17 8B 46 08 89 45 00 8B 44 24 40 89 45 04 8B 44 24 60 89 28 89 D8 EB 0F 55 E8 ?? ?? ?? ?? 83 C8 FF 5A EB 03 83 C8 FF 83 C4 44 5B 5E 5F 5D C3 }
	condition:
		$pattern
}
...
rule __GI_strftime_0cd607c9f7efea62e578acd49966d03f {
	meta:
...
rule vwarn_work_9389c213fbbbfe79b27bb91909fd7cf8 {
	meta:
		aliases = "vwarn_work"
		size = "187"
		objfiles = "err@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 50 89 C7 89 D5 BB ?? ?? ?? ?? 85 C9 74 1B E8 ?? ?? ?? ?? 6A 40 8D 54 24 04 52 FF 30 E8 ?? ?? ?? ?? BB ?? ?? ?? ?? 83 C4 0C A1 ?? ?? ?? ?? 8B 70 34 85 F6 75 24 83 C0 38 50 68 ?? ?? ?? ?? 8D 44 24 48 50 E8 ?? ?? ?? ?? A1 ?? ?? ?? ?? 83 C0 38 50 E8 ?? ?? ?? ?? 83 C4 10 FF 35 ?? ?? ?? ?? 68 ?? ?? ?? ?? FF 35 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 0C 85 FF 74 13 55 57 FF 35 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 EB 02 83 C4 0C 89 E0 50 53 FF 35 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 0C 85 F6 75 0E 6A 01 8D 44 24 44 50 E8 ?? ?? ?? ?? 58 5A 83 C4 50 5B 5E 5F 5D C3 }
	condition:
		$pattern
}
...
rule initshells_1aef19d4d15b176398e873c02626af16 {
	meta:
		aliases = "initshells"
		size = "291"
		objfiles = "usershell@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 58 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C7 5D 58 B8 ?? ?? ?? ?? 85 FF 0F 84 EF 00 00 00 57 E8 ?? ?? ?? ?? 8D 54 24 04 52 50 E8 ?? ?? ?? ?? 83 C4 0C 40 0F 84 C3 00 00 00 8B 44 24 2C 40 50 E8 ?? ?? ?? ?? A3 ?? ?? ?? ?? 5E 85 C0 0F 84 AA 00 00 00 6A 04 8B 44 24 30 BA 03 00 00 00 89 D1 31 D2 F7 F1 50 E8 ?? ?? ?? ?? A3 ?? ?? ?? ?? 59 5B 85 C0 0F 84 84 00 00 00 6A 02 57 E8 ?? ?? ?? ?? 8B 35 ?? ?? ?? ?? 8B 1D ?? ?? ?? ?? 8B 6C 24 34 58 5A EB 3A 43 8A 03 3C 23 74 33 3C 2F 74 06 84 C0 75 F1 EB 29 84 C0 74 25 89 1E EB 01 43 8A 0B 0F B6 D1 A1 ?? ?? ?? ?? F6 04 }
	condition:
		$pattern
}

rule gamma_r_c498904ebf352a16b39ba8037b79f02c {
	meta:
		aliases = "__ieee754_lgamma_r, lgamma_r, gamma_r"
		size = "1649"
		objfiles = "e_lgamma_r@libm.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 5C 8B 6C 24 78 DD 44 24 70 DD 54 24 28 8B 44 24 2C 89 44 24 40 C7 45 00 01 00 00 00 89 C6 81 E6 FF FF FF 7F 81 FE FF FF EF 7F 7E 09 D9 C0 DE C9 E9 2E 06 00 00 DD D8 8B 7C 24 28 89 F0 09 F8 74 58 81 FE FF FF 8F 3B 7F 36 83 7C 24 40 00 79 19 C7 45 00 FF FF FF FF DD 44 24 70 D9 E0 DD 54 24 70 83 EC 08 DD 1C 24 EB 08 FF 74 24 74 FF 74 24 74 E8 ?? ?? ?? ?? D9 E0 59 5B E9 E4 05 00 00 83 7C 24 40 00 78 0B D9 EE DD 5C 24 38 E9 E9 01 00 00 81 FE FF FF 2F 43 7E 07 D9 EE E9 88 01 00 00 DD 44 24 70 DD 5C 24 20 8B 5C 24 24 81 E3 FF FF FF 7F 81 FB FF FF CF 3F 7F 27 6A 00 6A 00 6A 00 DD 44 }
	condition:
		$pattern
}

rule __GI_strptime_b6f524552e0469fbe871e3b07d6fc4cd {
	meta:
		aliases = "strptime, __GI_strptime"
		size = "954"
		objfiles = "strptime@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 60 31 C0 C7 44 84 14 00 00 00 80 40 83 F8 0C 7E F2 8B 7C 24 78 C7 44 24 08 00 00 00 00 8A 07 84 C0 75 47 83 7C 24 08 00 75 32 83 7C 24 2C 07 75 08 C7 44 24 2C 00 00 00 00 31 D2 8B 44 94 14 3D 00 00 00 80 74 07 8B 4C 24 7C 89 04 91 42 83 FA 07 7E E8 8B 44 24 74 E9 50 03 00 00 FF 4C 24 08 8B 5C 24 08 8B 7C 9C 48 EB B3 3C 25 0F 85 FF 02 00 00 47 8A 07 3C 25 0F 84 F4 02 00 00 3C 4F 74 08 B1 3F 3C 45 75 0E EB 04 B0 40 EB 02 B0 80 88 C1 83 C9 3F 47 8A 17 84 D2 0F 84 0B 03 00 00 88 D0 83 C8 20 83 E8 61 3C 19 0F 87 FB 02 00 00 0F B6 C2 8A 90 ?? ?? ?? ?? 21 D1 80 F9 3E 0F 87 E7 02 00 }
	condition:
		$pattern
}

rule lckpwdf_8882c6a28a20ba68511d2cb8b2f0a33a {
	meta:
		aliases = "lckpwdf"
		size = "320"
		objfiles = "lckpwdf@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 60 83 C8 FF 83 3D ?? ?? ?? ?? FF 0F 85 21 01 00 00 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 38 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 68 01 00 08 00 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? A3 ?? ?? ?? ?? 83 C4 18 83 F8 FF 0F 84 D3 00 00 00 6A 01 6A 02 50 E8 ?? ?? ?? ?? 31 F6 8D 54 24 14 89 D7 89 F0 AB AB AB AB AB C7 44 24 14 ?? ?? ?? ?? C7 44 24 20 FF FF FF FF C7 44 24 24 FF FF FF FF 8D 44 24 28 50 52 6A 0E E8 ?? ?? ?? ?? C7 44 24 68 00 00 00 00 C7 44 24 6C 00 00 00 00 6A 0E 8D 5C 24 6C 53 E8 ?? ?? ?? ?? 83 C4 20 8D 6C 24 58 55 53 6A 01 E8 ?? ?? ?? ?? 6A 0F E8 ?? ?? ?? ?? 8D 54 }
	condition:
		$pattern
}
...
rule _time_mktime_tzi_763e114fa6579812155001e3743e25f4 {
	meta:
...
rule __gen_tempname_10c5211ccb106d3c672b2cc86957395f {
	meta:
		aliases = "__gen_tempname"
		size = "605"
		objfiles = "tempname@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 83 EC 7C E8 ?? ?? ?? ?? 89 44 24 04 8B 00 89 44 24 10 FF B4 24 90 00 00 00 E8 ?? ?? ?? ?? 5B 83 F8 05 76 2D 8B 94 24 90 00 00 00 8D 44 10 FA 89 44 24 08 68 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 5A 59 85 C0 75 0D C7 44 24 0C 00 00 00 00 E9 E5 01 00 00 8B 4C 24 04 C7 01 16 00 00 00 E9 EE 01 00 00 6A 00 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C7 5E 5D 85 C0 79 17 68 00 08 00 00 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C7 59 5B 85 C0 78 21 6A 06 8D 5C 24 7A 53 57 E8 ?? ?? ?? ?? 89 C3 57 E8 ?? ?? ?? ?? 83 C4 10 83 FB 06 0F 84 AB 00 00 00 6A 00 8D 44 24 20 50 E8 ?? ?? ?? ?? 8B 15 ?? ?? ?? ?? 8B 0D ?? ?? ?? ?? }
	condition:
		$pattern
}
...
rule rwlock_have_already_b9bdbd1167c95b254050229fe9b4fad2 {
	meta:
		aliases = "rwlock_have_already"
		size = "164"
		objfiles = "rwlock@libpthread.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 89 C7 89 D6 89 CD 8B 18 83 7A 18 01 74 06 31 C9 31 D2 EB 6F 85 DB 75 07 E8 ?? ?? ?? ?? 89 C3 8B 93 48 01 00 00 EB 07 39 72 04 74 08 8B 12 85 D2 75 F5 EB 04 85 D2 75 4F 83 BB 50 01 00 00 00 7F 46 8B 93 4C 01 00 00 85 D2 74 0A 8B 02 89 83 4C 01 00 00 EB 0A 6A 0C E8 ?? ?? ?? ?? 89 C2 58 85 D2 74 18 C7 42 08 01 00 00 00 89 72 04 8B 83 48 01 00 00 89 02 89 93 48 01 00 00 83 FA 01 19 C9 83 E1 01 31 F6 EB 07 31 C9 BE 01 00 00 00 8B 44 24 14 89 08 89 55 00 89 1F 89 F0 5B 5E 5F 5D C3 }
	condition:
		$pattern
}
...
rule __GI_sgetspent_r_637203732147f55787736c71f6fee03f {
	meta:
...
rule __GI_ether_aton_r_deb35b8732eca94dfb71c23bd06567cb {
	meta:
		aliases = "ether_aton_r, __GI_ether_aton_r"
		size = "134"
		objfiles = "ether_addr@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 57 56 53 8B 5C 24 14 8B 7C 24 18 31 ED EB 66 8A 13 83 CA 20 8D 4A D0 80 F9 09 76 07 8D 42 9F 3C 05 77 59 89 CE 80 FA 39 76 03 8D 72 A9 8A 43 01 83 C3 02 83 FD 05 74 04 3C 3A EB 02 84 C0 74 2F 88 C2 83 CA 20 8D 4A D0 80 F9 09 76 07 8D 42 9F 3C 05 77 28 80 FA 39 76 03 8D 4A A9 89 F0 C1 E0 04 8D 34 01 83 FD 05 74 06 80 3B 3A 75 0E 43 89 F0 88 04 2F 45 83 FD 05 7E 95 EB 02 31 FF 89 F8 5B 5E 5F 5D C3 }
	condition:
		$pattern
}
...
rule __GI_asctime_r_64798aad599e2560d05434ec201b33f9 {
	meta:
...
rule __GI_xprt_register_e12e9d1edbf21891bc5c69b530c903ad {
	meta:
...
rule __GI_getcwd_740ab8eeda78801b355fafd24103699f {
	meta:
...
rule __GI_fgets_unlocked_180b4b978508fa29c98a6e5c2ff7cbd9 {
	meta:
...
rule __GI_getgrgid_r_23a0aeab15032686cb04abc9bea484b7 {
	meta:
...
rule __GI_getgrnam_r_e612ed03d06dd4fd4054bcb61abc8918 {
	meta:
		aliases = "__GI_getpwnam_r, __GI_getspnam_r, getgrnam_r, getspnam_r, getpwnam_r, __GI_getgrnam_r"
		size = "135"
...
rule _Unwind_FindEnclosingFunction_140843736b3275eead73f559484bfef9 {
	meta:
		aliases = "_Unwind_GetDataRelBase, __gthread_active_p, __udiv_w_sdiv, _Unwind_GetTextRelBase, _Unwind_GetRegionStart, _Unwind_FindEnclosingFunction"
		size = "7"
		objfiles = "unwind_sjlj@libgcc_eh.a, _udiv_w_sdiv@libgcc.a, gthr_gnat@libgcc_eh.a"
	strings:
...
rule __register_frame_info_e980501d6f7808850243e5ed8c240a0e {
	meta:
...
rule daemon_3f388c42fd6fcf4af33fb86313c1bf40 {
	meta:
		aliases = "daemon"
		size = "136"
		objfiles = "daemon@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 89 E5 53 E8 ?? ?? ?? ?? 83 F8 FF 74 62 85 C0 74 67 6A 00 E8 ?? ?? ?? ?? 83 7D 08 00 75 0B 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 58 83 7D 0C 00 75 45 6A 02 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C3 59 58 83 FB FF 74 30 6A 00 53 E8 ?? ?? ?? ?? 6A 01 53 E8 ?? ?? ?? ?? 6A 02 53 E8 ?? ?? ?? ?? 83 C4 18 83 FB 02 7E 10 53 E8 ?? ?? ?? ?? 31 C0 5A EB 13 83 C8 FF EB 0E 31 C0 EB 0A E8 ?? ?? ?? ?? 40 75 98 EB ED 8B 5D FC C9 C3 }
	condition:
		$pattern
}
...
rule __GI_execlp_004def64f407b65fc7303d0c7e5ffc52 {
	meta:
...
rule __new_sem_wait_b95d29974a91fbe38866e019058a87ff {
	meta:
...
rule __GI_pthread_setcanceltype_bc7f2b0553f5359939c53394bd7ff33d {
	meta:
...
rule __pthread_cleanup_pop_restore_2004df8f172439abb358435fba4b8280 {
	meta:
...
rule gaih_inet_30cf6b86daa074bf9c1dfa36117eed1d {
	meta:
		aliases = "gaih_inet"
		size = "2388"
		objfiles = "getaddrinfo@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 89 E5 57 56 53 81 EC 88 00 00 00 8D 45 D4 89 45 F0 C7 45 EC 00 00 00 00 8B 55 10 8B 42 04 85 C0 74 0F C7 85 78 FF FF FF 00 00 00 00 83 F8 0A 75 16 8B 4D 10 8B 19 83 F3 08 C1 EB 03 F7 D3 83 E3 01 89 9D 78 FF FF FF 31 C0 8D 7D D4 AB AB AB AB 8B 7D 10 8B 4F 0C 85 C9 75 0D 83 7F 08 00 75 07 EB 5A 83 C3 07 EB 05 BB ?? ?? ?? ?? 8A 43 03 88 85 77 FF FF FF 84 C0 74 2C 8B 7D 10 8B 57 08 85 D2 74 07 0F BE 03 39 C2 75 D8 85 C9 74 0E F6 43 02 02 75 08 0F BE 43 01 39 C1 75 C6 80 BD 77 FF FF FF 00 75 1C B8 07 01 00 00 8B 55 10 83 7A 08 00 0F 85 94 08 00 00 E9 78 08 00 00 BB ?? ?? ?? ?? 83 7D 0C 00 0F 84 }
	condition:
		$pattern
}
...
rule __GI_glob_abae3945ca4dfa410e51354622de102e {
	meta:
		aliases = "glob, __GI_glob"
		size = "1301"
		objfiles = "glob@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 89 E5 57 56 53 81 EC D8 00 00 00 8B 7D 14 83 7D 08 00 74 0D 85 FF 74 09 F7 45 0C 00 81 FF FF 74 13 E8 ?? ?? ?? ?? C7 00 16 00 00 00 83 CA FF E9 D6 04 00 00 8B 45 0C 83 E0 08 89 85 24 FF FF FF 75 07 C7 47 08 00 00 00 00 6A 2F FF 75 08 E8 ?? ?? ?? ?? 5B 5E 89 C6 85 C0 75 38 F7 45 0C 00 50 00 00 0F 84 C9 00 00 00 8B 55 08 80 3A 7E 0F 85 BD 00 00 00 52 E8 ?? ?? ?? ?? 59 89 85 30 FF FF FF 8B 5D 08 C7 85 2C FF FF FF 00 00 00 00 E9 B6 00 00 00 3B 45 08 75 1E 8B 4D 08 41 89 8D 2C FF FF FF BB ?? ?? ?? ?? C7 85 30 FF FF FF 01 00 00 00 E9 93 00 00 00 2B 45 08 89 85 30 FF FF FF 83 C0 13 83 E0 FC 29 C4 }
	condition:
		$pattern
}
...
rule glob64_3afc1259ee2630cc40193defc5e8e2cf {
	meta:
		aliases = "__GI_glob64, glob64"
		size = "1301"
		objfiles = "glob64@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 89 E5 57 56 53 81 EC E8 00 00 00 8B 7D 14 83 7D 08 00 74 0D 85 FF 74 09 F7 45 0C 00 81 FF FF 74 13 E8 ?? ?? ?? ?? C7 00 16 00 00 00 83 CA FF E9 D6 04 00 00 8B 45 0C 83 E0 08 89 85 14 FF FF FF 75 07 C7 47 08 00 00 00 00 6A 2F FF 75 08 E8 ?? ?? ?? ?? 5B 5E 89 C6 85 C0 75 38 F7 45 0C 00 50 00 00 0F 84 C9 00 00 00 8B 55 08 80 3A 7E 0F 85 BD 00 00 00 52 E8 ?? ?? ?? ?? 59 89 85 20 FF FF FF 8B 5D 08 C7 85 1C FF FF FF 00 00 00 00 E9 B6 00 00 00 3B 45 08 75 1E 8B 4D 08 41 89 8D 1C FF FF FF BB ?? ?? ?? ?? C7 85 20 FF FF FF 01 00 00 00 E9 93 00 00 00 2B 45 08 89 85 20 FF FF FF 83 C0 13 83 E0 FC 29 C4 }
	condition:
		$pattern
}
...
rule __GI_pthread_cond_timedwait_650834ba2f86f073567506b298e947e2 {
	meta:
...
rule if_nameindex_ebb2de93c5c0abe22b9929065865d993 {
	meta:
		aliases = "__GI_if_nameindex, if_nameindex"
		size = "377"
		objfiles = "if_index@libc.a"
	strings:
		$pattern = { ( CC | 55 ) 89 E5 57 56 53 83 EC 18 E8 ?? ?? ?? ?? 89 45 DC C7 45 E4 00 00 00 00 85 C0 0F 88 4E 01 00 00 C7 45 F0 00 00 00 00 B9 80 00 00 00 8D 75 EC 8D 1C 09 8D 43 12 83 E0 FC 29 C4 8D 54 24 0F 83 E2 F0 8D 04 1A 3B 45 F0 75 02 01 CB 89 55 F0 89 5D EC 56 68 12 89 00 00 FF 75 DC E8 ?? ?? ?? ?? 83 C4 0C 85 C0 79 14 FF 75 DC E8 ?? ?? ?? ?? C7 45 E4 00 00 00 00 E9 F3 00 00 00 8B 45 EC 39 D8 75 04 89 C1 EB AA C1 E8 05 89 45 E0 8D 04 C5 08 00 00 00 50 E8 ?? ?? ?? ?? 89 45 E4 59 31 FF 85 C0 0F 85 A3 00 00 00 FF 75 DC E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? C7 00 69 00 00 00 E9 AF 00 00 00 89 FB C1 E3 05 03 5D F0 8B 45 }
	condition:
		$pattern
}
...
rule __GI_nan_9538f1961499397ad5bf685562440dcc {
	meta:
...
rule __enable_execute_stack_939a034664c2b3234964a30decbd1f50 {
	meta:
		aliases = "__gcov_init, __gnat_default_unlock, __clear_cache, __gcov_merge_single, __gcov_merge_add, __gcov_flush, __gcov_merge_delta, __gnat_default_lock, __enable_execute_stack"
		size = "5"
		objfiles = "_gcov@libgcov.a, gthr_gnat@libgcc_eh.a, _gcov_merge_single@libgcov.a, _gcov_merge_delta@libgcov.a, _gcov_merge_add@libgcov.a"
	strings:
...
rule thread_self_dd5dae98b7dbe953d0d7e291f00b93db {
	meta:
		aliases = "thread_self"
		size = "68"
		objfiles = "spinlock@libpthread.a, rwlock@libpthread.a, condvar@libpthread.a, cancel@libpthread.a, pthread@libpthread.a"
	strings:
...
rule __floatdidf_7e5b38da17692b5cd5a196f724e46232 {
	meta:
...
rule tmpfile64_ec5e35392bd89df16638e8945983bded {
	meta:
		aliases = "tmpfile, tmpfile64"
		size = "101"
		objfiles = "tmpfile@libc.a"
	strings:
		$pattern = { ( CC | 56 ) 53 81 EC 00 10 00 00 68 ?? ?? ?? ?? 6A 00 68 FF 0F 00 00 8D 5C 24 0D 53 E8 ?? ?? ?? ?? 83 C4 10 85 C0 75 33 6A 00 53 E8 ?? ?? ?? ?? 89 C6 5A 59 85 C0 78 23 53 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? 56 E8 ?? ?? ?? ?? 89 C3 83 C4 0C 85 C0 75 0B 56 E8 ?? ?? ?? ?? 58 EB 02 31 DB 89 D8 81 C4 00 10 00 00 5B 5E C3 }
	condition:
		$pattern
}
...
rule wcsxfrm_bd47bfe08ee0bf208651257e88d20352 {
	meta:
		aliases = "__GI_wcsxfrm, __wcslcpy, wcsxfrm"
		size = "62"
		objfiles = "wcslcpy@libc.a"
	strings:
		$pattern = { ( CC | 56 ) 53 83 EC 04 8B 4C 24 10 8B 74 24 14 8B 44 24 18 8D 58 FF 85 C0 75 04 89 E1 31 DB 89 F2 EB 0B 85 DB 74 04 4B 83 C1 04 83 C2 04 8B 02 89 01 85 C0 75 ED 29 F2 C1 FA 02 89 D0 5A 5B 5E C3 }
	condition:
		$pattern
}

rule __GI_strxfrm_74a18f0f542c50646d96143445c1d718 {
	meta:
...
rule __GI_strlcat_817ea6dbea9fde13c64419bd4f24d5a6 {
	meta:
...
rule __GI_memrchr_5342f1f56d7cc99524a57c3d479d15d7 {
	meta:
...
rule __GI_fputc_unlocked_b15a6f9755d891f780cb639030287476 {
	meta:
		aliases = "__fputc_unlocked, putc_unlocked, __GI_putc_unlocked, __GI___fputc_unlocked, fputc_unlocked, __GI_fputc_unlocked"
		size = "181"
...
rule erand48_r_1dcbb47d4f0a5905d425bd85778ec04d {
	meta:
		aliases = "__GI_erand48_r, erand48_r"
		size = "135"
		objfiles = "erand48_r@libc.a"
	strings:
		$pattern = { ( CC | 56 ) 53 83 EC 08 C7 04 24 00 00 00 00 C7 44 24 04 00 00 00 00 8B 74 24 14 FF 74 24 18 56 E8 ?? ?? ?? ?? 5B 5A 83 CA FF 85 C0 78 55 8B 54 24 04 81 E2 FF FF 0F 00 81 CA 00 00 F0 3F 89 54 24 04 66 8B 5E 02 89 D9 66 C1 E9 0C 0F B7 C9 0F B7 46 04 C1 E0 04 09 C1 81 E2 00 00 F0 FF 09 CA 89 54 24 04 C1 E3 14 0F B7 06 C1 E0 04 09 C3 89 1C 24 DD 04 24 DC 25 ?? ?? ?? ?? 8B 44 24 1C DD 18 31 D2 89 D0 5A 59 5B 5E C3 }
	condition:
		$pattern
}
...
rule __GI_getnetent_3291f869854fbd9357955f772834a586 {
	meta:
		aliases = "getnetent, __GI_getnetent"
		size = "364"
		objfiles = "getnetent@libc.a"
	strings:
		$pattern = { ( CC | 56 ) 53 83 EC 10 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 10 83 3D ?? ?? ?? ?? 00 75 1E 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? A3 ?? ?? ?? ?? 59 5B 85 C0 0F 84 09 01 00 00 83 3D ?? ?? ?? ?? 00 75 19 68 01 10 00 00 E8 ?? ?? ?? ?? A3 ?? ?? ?? ?? 5A 85 C0 75 05 E8 ?? ?? ?? ?? FF 35 ?? ?? ?? ?? 68 00 10 00 00 FF 35 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C3 83 C4 0C 85 C0 0F 84 C4 00 00 00 80 38 23 74 B6 BA ?? ?? ?? ?? E8 ?? ?? ?? ?? 85 C0 74 A8 C6 00 00 89 1D ?? ?? ?? ?? BA ?? ?? ?? ?? 89 D8 E8 ?? ?? ?? ?? 85 C0 74 8F C6 00 00 8D 58 01 EB 01 43 8A }
	condition:
		$pattern
}

rule getutline_625d372f6e4a19f393496cb7db38c10b {
	meta:
		aliases = "getutline"
		size = "105"
		objfiles = "utent@libc.a"
	strings:
		$pattern = { ( CC | 56 ) 53 83 EC 10 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 10 8B 74 24 1C 83 C6 08 EB 1B 8B 03 83 E8 06 66 83 F8 01 77 10 56 8D 43 08 50 E8 ?? ?? ?? ?? 5A 59 85 C0 74 0B E8 ?? ?? ?? ?? 89 C3 85 C0 75 DA 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 89 D8 83 C4 18 5B 5E C3 }
	condition:
		$pattern
}
...
rule __GI_getutid_8130b8e244ff5d1e14286cdad7cf68d7 {
	meta:
...
rule __libc_pselect_a4efd7ed7cd9a93dcd096aa8769322a6 {
	meta:
...
rule __GI_fgetc_961aa6f62fbab4254bd782098cead616 {
	meta:
...
rule __GI_log_c6a025aea17bd7ca73846953b8d35a4b {
	meta:
...
rule __GI_pmap_set_6ed1422dbe7b50956aa54d8905d890b1 {
	meta:
...
rule __GI_remquo_37d668307a52abcd085a57d968f0842c {
	meta:
...
rule opendir_864cd6c19690f8731160095061a4f5a2 {
	meta:
		aliases = "__GI_opendir, opendir"
		size = "108"
		objfiles = "opendir@libc.a"
	strings:
		$pattern = { ( CC | 56 ) 53 83 EC 58 68 00 08 09 00 FF 74 24 68 E8 ?? ?? ?? ?? 89 C3 5E 58 31 F6 85 DB 78 47 89 E0 50 53 E8 ?? ?? ?? ?? 5A 59 85 C0 79 08 53 E8 ?? ?? ?? ?? EB 2F 6A 01 6A 02 53 E8 ?? ?? ?? ?? 8B 54 24 3C 89 D8 E8 ?? ?? ?? ?? 89 C6 83 C4 0C 85 C0 75 12 53 E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? C7 00 0C 00 00 00 59 89 F0 83 C4 58 5B 5E C3 }
	condition:
		$pattern
}

rule __GI_cbrt_8abe9979e3fd600886fbac1bc2072d44 {
	meta:
...
rule __libc_pread64_5c22623e9d600939c2de16f66411f0fb {
	meta:
...
rule __libc_pwrite64_a8be20e122709c174249db17691b0073 {
	meta:
...
rule __new_sem_trywait_a6f922629b79da2a10e603424c0a845e {
	meta:
...
rule __GI_pthread_cond_broadcast_c28935e3317bd6f5b11ff00531d74b39 {
	meta:
...
rule __GI_globfree_0870b4fc6a52ef2349f3f72211a53a44 {
	meta:
		aliases = "__GI_globfree64, globfree, globfree64, __GI_globfree"
		size = "62"
...
rule __GI_fputws_unlocked_744844744fe0a8180a2fb361bd415a0d {
	meta:
...
rule __GI_herror_adf09dbd1995843166ce74cf401706b6 {
	meta:
...
rule svctcp_destroy_e35d14c0356c14b6fc27392a9786a731 {
	meta:
...
rule __GI__authenticate_0999279fae2b248ecd7c02fbbfa8ac3a {
	meta:
...
rule __pthread_attr_setguardsize_718c14e1ca8c45fd5d5f688b60051f9c {
	meta:
...
rule __GI_pthread_attr_setschedpara_94f938311ef709a0464faae3dd09d489 {
	meta:
...
rule setegid_f9240865b21bb1c7aef5a0b5ec1eca9a {
	meta:
...
rule __GI_pthread_getschedparam_77ef64a2070db0bb78be7af5b8687a28 {
	meta:
...
rule __pthread_mutex_lock_0bfb688b1ffcfb5f8c76c85e2d7b4e0e {
	meta:
...
rule __GI_xdr_opaque_auth_38d77b29fbd773ce78520bc9bd550e18 {
	meta:
...
rule __GI_xdr_replymsg_ab31c401c99117ca3b8d2f8e0cbdfeb9 {
	meta:
...
rule __GI_xdr_rejected_reply_e1d2e2de088aab812bb43573391bf507 {
	meta:
...
rule __GI_xdr_callmsg_6b87164d49e448a74396616023513710 {
	meta:
...
rule longjmp_80874c12c0923f06960ef9b6fc6b2db8 {
	meta:
...
rule __GI_tfind_ed002546cc2b66f9dd6d5a38a03ab767 {
	meta:
...
rule updwtmp_7b5f864371f42b037acbf9c2d16105ff {
	meta:
		aliases = "updwtmp"
		size = "84"
		objfiles = "wtent@libc.a"
	strings:
		$pattern = { ( CC | 56 ) 53 8B 74 24 10 68 01 04 00 00 FF 74 24 10 E8 ?? ?? ?? ?? 89 C3 58 5A 85 DB 78 35 6A 00 6A 01 53 E8 ?? ?? ?? ?? 83 C4 0C 85 C0 75 24 68 80 01 00 00 56 53 E8 ?? ?? ?? ?? 6A 00 6A 00 53 E8 ?? ?? ?? ?? 83 C4 18 89 5C 24 0C 5B 5E E9 ?? ?? ?? ?? 5B 5E C3 }
	condition:
		$pattern
}

rule __GI_hcreate_r_846ac91b930756d157f41a79795c54d6 {
	meta:
...
rule __GI_fgetspent_r_e1f38611a4b1ad9d822de85e67acd783 {
	meta:
		aliases = "__GI_fgetgrent_r, fgetspent_r, fgetgrent_r, __GI_fgetpwent_r, fgetpwent_r, __GI_fgetspent_r"
		size = "51"
...
rule sethostid_2f060b77119616a65473be8a6c3d6750 {
	meta:
		aliases = "sethostid"
		size = "103"
		objfiles = "hostid@libc.a"
	strings:
		$pattern = { ( CC | 56 ) 53 E8 ?? ?? ?? ?? 85 C0 75 09 E8 ?? ?? ?? ?? 85 C0 74 12 E8 ?? ?? ?? ?? C7 00 01 00 00 00 BE 01 00 00 00 EB 3C 68 A4 01 00 00 6A 41 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C6 83 C4 0C 85 C0 78 22 6A 04 8D 44 24 10 50 56 E8 ?? ?? ?? ?? 83 C4 0C 31 DB 83 F8 04 0F 94 C3 4B 56 E8 ?? ?? ?? ?? 89 DE 58 89 F0 5B 5E C3 }
	condition:
		$pattern
}
...
rule res_sync_func_2b6f8025adb3e1ec324e112efbb38501 {
	meta:
		aliases = "res_sync_func"
		size = "92"
		objfiles = "res_init@libc.a"
	strings:
		$pattern = { ( CC | 56 ) 53 E8 ?? ?? ?? ?? 89 C6 81 3D ?? ?? ?? ?? ?? ?? ?? ?? 74 34 0F B6 40 60 39 05 ?? ?? ?? ?? 76 05 A3 ?? ?? ?? ?? 8B 1D ?? ?? ?? ?? EB 18 6B C3 1C 03 05 ?? ?? ?? ?? 6A 1C FF 74 9E 54 50 E8 ?? ?? ?? ?? 83 C4 0C 4B 79 E5 8A 46 52 A2 ?? ?? ?? ?? 8A 46 53 A2 ?? ?? ?? ?? 5B 5E C3 }
	condition:
		$pattern
}
...
rule __GI_ftell_824e97517a9adc96dee5b0400f95e7fa {
	meta:
...
rule logout_72e6509ced32b6841b69a29abe82f7d8 {
	meta:
		aliases = "logout"
		size = "150"
		objfiles = "logout@libutil.a"
	strings:
		$pattern = { ( CC | 57 ) 53 81 EC 80 01 00 00 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 5A 31 DB 40 74 73 E8 ?? ?? ?? ?? 66 C7 04 24 07 00 6A 20 FF B4 24 90 01 00 00 8D 5C 24 08 8D 44 24 10 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 89 C3 83 C4 10 85 C0 74 3B 31 C0 B9 08 00 00 00 8D 7B 2C F3 AB B9 40 00 00 00 8D 7B 4C F3 AB 6A 00 8D 83 54 01 00 00 50 E8 ?? ?? ?? ?? 66 C7 03 08 00 53 E8 ?? ?? ?? ?? 83 C4 0C BB 01 00 00 00 85 C0 75 02 31 DB E8 ?? ?? ?? ?? 89 D8 81 C4 80 01 00 00 5B 5F C3 }
	condition:
		$pattern
}

rule sigqueue_785645fa3db4b17b6166f7f21b0d29be {
	meta:
		aliases = "sigqueue"
		size = "129"
...
rule __libc_open_c7ea7255401774be8b6f1b668400b2fd {
	meta:
		aliases = "__GI_open, open, __libc_open"
		size = "68"
		objfiles = "open@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 53 83 EC 04 8B 7C 24 10 8B 4C 24 14 31 D2 F6 C1 40 74 0B 8D 44 24 1C 89 04 24 8B 54 24 18 53 89 FB B8 05 00 00 00 CD 80 5B 89 C3 81 FB 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DB 89 18 83 C8 FF 5A 5B 5F C3 }
	condition:
		$pattern
}

rule __GI_fcntl_cb85df3c9cec5dbfb22e733ab1ae2fc2 {
	meta:
		aliases = "fcntl, __libc_fcntl, __GI_fcntl"
		size = "82"
		objfiles = "__syscall_fcntl@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 53 83 EC 04 8B 7C 24 10 8B 4C 24 14 8D 44 24 1C 89 04 24 8B 54 24 18 8D 41 F4 83 F8 02 77 0D 52 51 57 E8 ?? ?? ?? ?? 83 C4 0C EB 21 53 89 FB B8 37 00 00 00 CD 80 5B 89 C3 81 FB 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DB 89 18 83 C8 FF 5A 5B 5F C3 }
	condition:
		$pattern
}

rule truncate64_0133389d7fd30a1847cf7519aa9b495d {
	meta:
		aliases = "truncate64"
		size = "76"
		objfiles = "truncate64@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 53 83 EC 08 8B 7C 24 14 8B 44 24 18 8B 54 24 1C 89 D0 89 C2 C1 FA 1F 89 04 24 89 54 24 04 8B 4C 24 18 8B 14 24 53 89 FB B8 C1 00 00 00 CD 80 5B 89 C7 81 FF 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DF 89 38 83 C8 FF 5A 59 5B 5F C3 }
	condition:
		$pattern
}

rule ftruncate64_b0167d389a19398907c8674cb6775610 {
	meta:
		aliases = "__GI_ftruncate64, ftruncate64"
		size = "76"
		objfiles = "ftruncate64@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 53 83 EC 08 8B 7C 24 14 8B 44 24 18 8B 54 24 1C 89 D0 89 C2 C1 FA 1F 89 04 24 89 54 24 04 8B 4C 24 18 8B 14 24 53 89 FB B8 C2 00 00 00 CD 80 5B 89 C7 81 FF 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DF 89 38 83 C8 FF 5A 59 5B 5F C3 }
	condition:
		$pattern
}
...
rule __GI_abort_139c6b4607c8bc8b054bc8446ec706b5 {
	meta:
...
rule gethostid_960f57a8d5cb0b2c67c12a451be4dcfc {
	meta:
		aliases = "gethostid"
		size = "160"
		objfiles = "hostid@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 53 83 EC 70 C7 44 24 6C 00 00 00 00 6A 00 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C7 5B 58 85 FF 78 1C 6A 04 8D 44 24 70 50 57 E8 ?? ?? ?? ?? 89 C3 57 E8 ?? ?? ?? ?? 83 C4 10 85 DB 7F 59 6A 40 8D 5C 24 0B 53 E8 ?? ?? ?? ?? 5A 59 85 C0 78 47 80 7C 24 07 00 74 40 31 C0 B9 08 00 00 00 8D 54 24 48 89 D7 F3 AB 8D 44 24 68 50 52 6A 00 53 E8 ?? ?? ?? ?? 83 C4 10 85 C0 75 1C 8B 54 24 68 85 D2 74 0D 8B 42 14 8B 40 04 C1 C8 10 89 44 24 6C 52 E8 ?? ?? ?? ?? 58 8B 44 24 6C 83 C4 70 5B 5F C3 }
	condition:
		$pattern
}
...
rule ptrace_5faa4dc8a11cf6c9328934242e9e2c02 {
	meta:
		aliases = "ptrace"
		size = "109"
		objfiles = "ptrace@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 08 8B 7C 24 18 8B 4C 24 1C 8B 54 24 20 8D 44 24 28 89 04 24 8B 74 24 24 8D 47 FF 83 F8 02 77 04 8D 74 24 04 53 89 FB B8 1A 00 00 00 CD 80 5B 89 C3 3D 00 F0 FF FF 76 0E E8 ?? ?? ?? ?? F7 DB 89 18 83 C8 FF EB 1C 85 C0 78 18 85 FF 74 14 83 FF 03 77 0F E8 ?? ?? ?? ?? C7 00 00 00 00 00 8B 44 24 04 5A 59 5B 5E 5F C3 }
	condition:
		$pattern
}

rule readahead_08a93312034bc33fa2f099045616da18 {
	meta:
		aliases = "readahead"
		size = "82"
		objfiles = "readahead@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 08 8B 7C 24 18 8B 74 24 24 8B 44 24 1C 8B 54 24 20 89 D0 89 C2 C1 FA 1F 89 04 24 89 54 24 04 8B 0C 24 8B 54 24 1C 53 89 FB B8 E1 00 00 00 CD 80 5B 89 C6 81 FE 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DE 89 30 83 C8 FF 5A 59 5B 5E 5F C3 }
	condition:
		$pattern
}
...
rule __GI_lseek64_205810a655cfee9b1a5deaa827939798 {
	meta:
...
rule puts_55443c024c68a281d36bbdfa6f046daa {
	meta:
		aliases = "puts"
		size = "112"
		objfiles = "puts@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 10 8B 35 ?? ?? ?? ?? 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 56 FF 74 24 24 E8 ?? ?? ?? ?? 89 C3 58 5A 83 FB FF 74 13 56 6A 0A E8 ?? ?? ?? ?? 59 5E 40 75 05 83 CB FF EB 01 43 85 FF 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 C4 10 5B 5E 5F C3 }
	condition:
		$pattern
}
...
rule fflush_5f7f31db11c722471dbdb0d32edf7212 {
	meta:
		aliases = "__GI_fflush, fflush"
		size = "104"
		objfiles = "fflush@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 10 8B 74 24 20 85 F6 74 48 81 FE ?? ?? ?? ?? 74 40 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 56 E8 ?? ?? ?? ?? 89 C3 5E 85 FF 75 19 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 5A 59 EB 09 56 E8 ?? ?? ?? ?? 89 C3 58 89 D8 83 C4 10 5B 5E 5F C3 }
	condition:
		$pattern
}
...
rule vfwprintf_60a2607956c0541e0917b2236406ac4a {
	meta:
		aliases = "__GI_vfwprintf, vfwprintf"
		size = "126"
		objfiles = "vfwprintf@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 10 8B 74 24 20 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 0F B7 06 25 40 08 00 00 3D 40 08 00 00 74 14 68 00 08 00 00 56 E8 ?? ?? ?? ?? 59 5B 83 CB FF 85 C0 75 13 FF 74 24 28 FF 74 24 28 56 E8 ?? ?? ?? ?? 89 C3 83 C4 0C 85 FF 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 C4 10 5B 5E 5F C3 }
	condition:
		$pattern
}

rule vfprintf_f9bad97e6029b81c8b2a1af067d3f755 {
	meta:
		aliases = "__GI_vfprintf, vfprintf"
		size = "126"
		objfiles = "vfprintf@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 10 8B 74 24 20 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 0F B7 06 25 C0 00 00 00 3D C0 00 00 00 74 14 68 80 00 00 00 56 E8 ?? ?? ?? ?? 59 5B 83 CB FF 85 C0 75 13 FF 74 24 28 FF 74 24 28 56 E8 ?? ?? ?? ?? 89 C3 83 C4 0C 85 FF 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 C4 10 5B 5E 5F C3 }
	condition:
		$pattern
}

rule fgetwc_114aa4157b4b9566192d34ac8548075b {
	meta:
		aliases = "__GI_fgetwc, getwc, __GI_fileno, fileno, fgetwc"
		size = "81"
		objfiles = "fgetwc@libc.a, fileno@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 10 8B 74 24 20 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 56 E8 ?? ?? ?? ?? 89 C3 59 85 FF 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 C4 10 5B 5E 5F C3 }
	condition:
		$pattern
}
...
rule feof_ee8c769ed69c0bc75a4468c9273866ab {
	meta:
		aliases = "feof"
		size = "77"
		objfiles = "feof@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 10 8B 74 24 20 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 8B 1E 85 FF 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 E0 04 83 C4 10 5B 5E 5F C3 }
	condition:
		$pattern
}

rule ferror_66a92744764cd15ca06e2570b0748d21 {
	meta:
		aliases = "ferror"
		size = "77"
		objfiles = "ferror@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 10 8B 74 24 20 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 8B 1E 85 FF 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 E0 08 83 C4 10 5B 5E 5F C3 }
	condition:
		$pattern
}

rule gets_72fc01243c417dc31be681ac1c2593ee {
	meta:
		aliases = "gets"
		size = "119"
		objfiles = "gets@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 10 8B 74 24 20 A1 ?? ?? ?? ?? 8B 78 34 85 FF 74 04 89 F3 EB 27 83 C0 38 50 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? A1 ?? ?? ?? ?? 83 C0 38 50 E8 ?? ?? ?? ?? 83 C4 10 EB D6 43 E8 ?? ?? ?? ?? 83 F8 FF 74 0A 88 03 3C 0A 75 EF 39 DE 75 04 31 F6 EB 03 C6 03 00 85 FF 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 F0 83 C4 10 5B 5E 5F C3 }
	condition:
		$pattern
}

rule __GI_fputws_7d61cd477258541349acca99333ccc23 {
	meta:
		aliases = "fputwc, fputs, fputws, putwc, __GI_fputs, __GI_fputws"
		size = "86"
		objfiles = "fputwc@libc.a, fputws@libc.a, fputs@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 10 8B 74 24 24 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 56 FF 74 24 24 E8 ?? ?? ?? ?? 89 C3 59 5E 85 FF 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 C4 10 5B 5E 5F C3 }
	condition:
		$pattern
}

rule fgetws_3ca1629c86e6c54373efd3d5f15d3e6e {
	meta:
		aliases = "fgets, __GI_fgets, fgetws"
		size = "91"
		objfiles = "fgets@libc.a, fgetws@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 10 8B 74 24 28 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 56 FF 74 24 28 FF 74 24 28 E8 ?? ?? ?? ?? 89 C3 83 C4 0C 85 FF 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 C4 10 5B 5E 5F C3 }
	condition:
		$pattern
}

rule fread_e64e0c6a030ccd39c5f6aff237204ea1 {
	meta:
		aliases = "__GI_fwrite, __GI_fread, fwrite, fread"
		size = "95"
		objfiles = "fwrite@libc.a, fread@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 10 8B 74 24 2C 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 56 FF 74 24 2C FF 74 24 2C FF 74 24 2C E8 ?? ?? ?? ?? 89 C3 83 C4 10 85 FF 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 89 D8 83 C4 10 5B 5E 5F C3 }
	condition:
		$pattern
}

rule __GI_closedir_0629d0d174d544e086a4ef705e0fdbff {
	meta:
...
rule __GI_putc_616f35da021ce09db47b602dae5f627e {
	meta:
...
rule getgrent_r_7ee86d6829de12dd3af398a00bb65158 {
	meta:
		aliases = "__GI_getspent_r, __GI_getgrent_r, __GI_getpwent_r, getspent_r, getpwent_r, getgrent_r"
		size = "161"
		objfiles = "getgrent_r@libc.a, getspent_r@libc.a, getpwent_r@libc.a"
	strings:
...
rule __GI_round_75f6e08d8d1bdff54aa7dccc2c14e7f8 {
	meta:
...
rule __GI_ftello64_d3fe9056786954a4d5c5e5dd9a6efdfe {
	meta:
		aliases = "ftello64, __GI_ftello64"
		size = "162"
		objfiles = "ftello64@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 18 8B 74 24 28 C7 44 24 10 00 00 00 00 C7 44 24 14 00 00 00 00 8B 7E 34 85 FF 75 1C 8D 5E 38 53 68 ?? ?? ?? ?? 8D 44 24 08 50 E8 ?? ?? ?? ?? 53 E8 ?? ?? ?? ?? 83 C4 10 0F B7 06 25 40 04 00 00 3D 40 04 00 00 0F 94 C0 0F B6 C0 40 50 8D 5C 24 14 53 56 E8 ?? ?? ?? ?? 83 C4 0C 85 C0 78 0D 53 56 E8 ?? ?? ?? ?? 59 5B 85 C0 79 10 C7 44 24 10 FF FF FF FF C7 44 24 14 FF FF FF FF 85 FF 75 0E 6A 01 8D 44 24 04 50 E8 ?? ?? ?? ?? 58 5A 8B 44 24 10 8B 54 24 14 83 C4 18 5B 5E 5F C3 }
	condition:
		$pattern
}

rule __GI_pmap_getport_20b24a7768ac42471fb589f801c3bcb1 {
	meta:
...
rule if_indextoname_b240b51025247b26716671e43b633c5f {
	meta:
		aliases = "if_indextoname"
		size = "105"
		objfiles = "if_index@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 20 E8 ?? ?? ?? ?? 89 C6 31 C0 85 F6 78 4F 8B 44 24 30 89 44 24 10 89 E3 53 68 10 89 00 00 56 E8 ?? ?? ?? ?? 83 C4 0C 85 C0 79 1D E8 ?? ?? ?? ?? 89 C7 8B 18 56 E8 ?? ?? ?? ?? 58 83 FB 13 75 02 B3 06 89 1F 31 C0 EB 15 56 E8 ?? ?? ?? ?? 6A 10 53 FF 74 24 40 E8 ?? ?? ?? ?? 83 C4 10 83 C4 20 5B 5E 5F C3 }
	condition:
		$pattern
}
...
rule __GI_rint_fdb847fb272d9befe638b0a435d08156 {
	meta:
		aliases = "rint, __GI_nearbyint, nearbyint, __GI_rint"
		size = "327"
		objfiles = "s_rint@libm.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 30 C7 04 24 00 00 00 00 C7 44 24 04 00 00 00 00 DD 44 24 40 DD 54 24 20 8B 5C 24 24 8B 74 24 20 89 DF C1 EF 1F 89 D8 C1 F8 14 25 FF 07 00 00 8D 88 01 FC FF FF 83 F9 13 0F 8F A3 00 00 00 85 C9 79 69 89 D8 25 FF FF FF 7F 09 F0 0F 84 E4 00 00 00 89 DA 81 E2 FF FF 0F 00 09 F2 DD 5C 24 18 89 D0 F7 D8 09 D0 C1 E8 0C 25 00 00 08 00 81 E3 00 00 FE FF 09 D8 89 44 24 1C DD 04 FD ?? ?? ?? ?? DD 44 24 18 D8 C1 DE E1 DD 54 24 10 DD 5C 24 08 C1 E7 1F 8B 44 24 14 25 FF FF FF 7F 09 C7 89 7C 24 0C DD 44 24 08 E9 84 00 00 00 DD D8 BA FF FF 0F 00 D3 FA 89 D0 21 D8 09 F0 74 7B D1 EA 89 D0 21 D8 09 }
	condition:
		$pattern
}
...
rule __GI_erfc_f5db6f4e77f020c949eb11d1f956ebc7 {
	meta:
...
rule modf_9595131186afc84fcab4a43db2b82ff8 {
	meta:
		aliases = "__GI_modf, modf"
		size = "400"
		objfiles = "s_modf@libm.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 50 C7 44 24 40 00 00 00 00 C7 44 24 44 00 00 00 00 C7 44 24 30 00 00 00 00 C7 44 24 34 00 00 00 00 C7 44 24 28 00 00 00 00 C7 44 24 2C 00 00 00 00 C7 44 24 18 00 00 00 00 C7 44 24 1C 00 00 00 00 C7 44 24 08 00 00 00 00 C7 44 24 0C 00 00 00 00 C7 04 24 00 00 00 00 C7 44 24 04 00 00 00 00 8B 7C 24 68 DD 44 24 60 DD 5C 24 48 8B 5C 24 4C 8B 74 24 48 89 D8 C1 F8 14 25 FF 07 00 00 8D 88 01 FC FF FF 83 F9 13 7F 7A 85 C9 79 1D 81 E3 00 00 00 80 89 5C 24 44 C7 44 24 40 00 00 00 00 DD 44 24 40 DD 1F E9 D6 00 00 00 BA FF FF 0F 00 D3 FA 89 D0 21 D8 09 F0 75 28 DD 44 24 60 DD 17 DD 5C 24 38 }
	condition:
		$pattern
}

rule pathconf_8f2c6e4e5685d8a9897216aa8f91e3cc {
	meta:
		aliases = "pathconf"
		size = "188"
		objfiles = "pathconf@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 58 8B 74 24 68 8B 44 24 6C 80 3E 00 75 10 E8 ?? ?? ?? ?? C7 00 02 00 00 00 E9 81 00 00 00 83 F8 13 77 0E FF 24 85 ?? ?? ?? ?? B8 20 00 00 00 EB 7F E8 ?? ?? ?? ?? C7 00 16 00 00 00 EB 61 B8 7F 00 00 00 EB 6B E8 ?? ?? ?? ?? 89 C3 8B 38 89 E0 50 56 E8 ?? ?? ?? ?? 5E 5A 85 C0 79 09 83 3B 26 75 3D 89 3B EB 32 8B 44 24 24 EB 44 31 C0 EB 40 89 E0 50 56 E8 ?? ?? ?? ?? 5A 59 85 C0 78 20 8B 44 24 10 25 00 F0 00 00 3D 00 80 00 00 74 1C 3D 00 60 00 00 75 09 EB 13 B8 FF 00 00 00 EB 11 83 C8 FF EB 0C B8 00 10 00 00 EB 05 B8 01 00 00 00 83 C4 58 5B 5E 5F C3 }
	condition:
		$pattern
}

rule fpathconf_1170f2c16375b6f1c42e76848bcfb994 {
	meta:
		aliases = "fpathconf"
		size = "196"
		objfiles = "fpathconf@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 83 EC 58 8B 74 24 68 8B 54 24 6C 85 F6 79 10 E8 ?? ?? ?? ?? C7 00 09 00 00 00 E9 8A 00 00 00 B8 7F 00 00 00 85 D2 0F 84 8E 00 00 00 8D 42 FF 83 F8 12 77 0E FF 24 85 ?? ?? ?? ?? B8 20 00 00 00 EB 78 E8 ?? ?? ?? ?? C7 00 16 00 00 00 EB 5A E8 ?? ?? ?? ?? 89 C3 8B 38 89 E0 50 56 E8 ?? ?? ?? ?? 5E 5A 85 C0 79 09 83 3B 26 75 3D 89 3B EB 32 8B 44 24 24 EB 44 31 C0 EB 40 89 E0 50 56 E8 ?? ?? ?? ?? 5A 59 85 C0 78 20 8B 44 24 10 25 00 F0 00 00 3D 00 80 00 00 74 1C 3D 00 60 00 00 75 09 EB 13 B8 FF 00 00 00 EB 11 83 C8 FF EB 0C B8 00 10 00 00 EB 05 B8 01 00 00 00 83 C4 58 5B 5E 5F C3 }
	condition:
		$pattern
}
...
rule __getutid_43e916e3b8bb2e52d8ec71865d3d88f2 {
	meta:
		aliases = "__getutid"
		size = "86"
		objfiles = "utent@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 89 C6 8D 78 28 EB 3B 8B 16 8D 42 FF 66 83 F8 03 77 05 66 39 13 74 36 66 83 FA 05 74 12 66 83 FA 08 74 0C 66 83 FA 06 74 06 66 83 FA 07 75 13 6A 04 57 8D 43 28 50 E8 ?? ?? ?? ?? 83 C4 0C 85 C0 74 0B E8 ?? ?? ?? ?? 89 C3 85 C0 75 BA 89 D8 5B 5E 5F C3 }
	condition:
		$pattern
}
...
rule fd_to_DIR_abc20176e06591131b183926d3ce5e22 {
	meta:
		aliases = "fd_to_DIR"
		size = "113"
		objfiles = "opendir@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 89 C7 89 D6 6A 30 E8 ?? ?? ?? ?? 5A 31 DB 85 C0 74 56 89 C3 89 38 C7 40 10 00 00 00 00 C7 40 08 00 00 00 00 C7 40 04 00 00 00 00 89 70 14 81 FE FF 01 00 00 77 07 C7 40 14 00 02 00 00 FF 73 14 6A 01 E8 ?? ?? ?? ?? 89 43 0C 5E 5F 85 C0 75 0B 53 E8 ?? ?? ?? ?? 31 DB 59 EB 0D 6A 00 8D 43 18 50 E8 ?? ?? ?? ?? 58 5A 89 D8 5B 5E 5F C3 }
	condition:
		$pattern
}
...
rule __GI_strcspn_28259d4bcb32e49be938f30092a7407a {
	meta:
...
rule __GI_strtok_r_4ac00f2890a2c65b7b9950d34fb7c621 {
	meta:
...
rule ___path_search_189c02a61df3c74e97d111225da0d5fc {
	meta:
		aliases = "___path_search"
		size = "188"
		objfiles = "tempname@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 8B 5C 24 18 8B 7C 24 1C 85 FF 74 1A 80 3F 00 74 15 57 E8 ?? ?? ?? ?? 59 89 C6 83 F8 05 76 11 BE 05 00 00 00 EB 0A BF ?? ?? ?? ?? BE 04 00 00 00 85 DB 75 38 B8 ?? ?? ?? ?? E8 ?? ?? ?? ?? BB ?? ?? ?? ?? 85 C0 75 25 53 53 E8 ?? ?? ?? ?? 59 5A 85 C0 74 0B 89 D8 E8 ?? ?? ?? ?? 85 C0 75 0D E8 ?? ?? ?? ?? C7 00 02 00 00 00 EB 2D 53 E8 ?? ?? ?? ?? 5A 89 C2 EB 01 4A 83 FA 01 76 07 80 7C 1A FF 2F 74 F3 8D 44 16 08 39 44 24 14 73 10 E8 ?? ?? ?? ?? C7 00 16 00 00 00 83 C8 FF EB 17 57 56 53 52 68 ?? ?? ?? ?? FF 74 24 24 E8 ?? ?? ?? ?? 31 C0 83 C4 18 5B 5E 5F C3 }
	condition:
		$pattern
}
...
rule __GI_wcscasecmp_7f8e02106d74ff02a4255971e2f05ff3 {
	meta:
...
rule __GI_remove_2364e6da4ad88f06e6e480342ee5c7c7 {
	meta:
...
rule getnetbyname_4b473903be8c6b00e3de015b7f4c2a8c {
	meta:
		aliases = "getnetbyname"
		size = "95"
		objfiles = "getnetbynm@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 8B 7C 24 10 0F BE 05 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 59 EB 29 57 FF 36 E8 ?? ?? ?? ?? 5B 5A 85 C0 74 26 8B 5E 04 EB 10 57 50 E8 ?? ?? ?? ?? 5A 59 85 C0 74 14 83 C3 04 8B 03 85 C0 75 EA E8 ?? ?? ?? ?? 89 C6 85 C0 75 CC 80 3D ?? ?? ?? ?? 00 75 05 E8 ?? ?? ?? ?? 89 F0 5B 5E 5F C3 }
	condition:
		$pattern
}
...
rule __GI_mknod_4d7d5c3414773db5eaaf7327ce1ea1ce {
	meta:
		aliases = "mknod, __GI_mknod"
		size = "54"
		objfiles = "mknod@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 8B 7C 24 10 8B 4C 24 14 8B 5C 24 18 89 DA 53 89 FB B8 0E 00 00 00 CD 80 5B 89 C7 81 FF 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DF 89 38 83 C8 FF 5B 5E 5F C3 }
	condition:
		$pattern
}
...
rule getnetbyaddr_1a698d8dc47a9dd44dd2f59bb502fa68 {
	meta:
		aliases = "getnetbyaddr"
		size = "68"
		objfiles = "getnetbyad@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 8B 7C 24 10 8B 74 24 14 0F BE 05 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 58 EB 0A 39 73 08 75 05 39 7B 0C 74 0B E8 ?? ?? ?? ?? 89 C3 85 C0 75 EB 80 3D ?? ?? ?? ?? 00 75 05 E8 ?? ?? ?? ?? 89 D8 5B 5E 5F C3 }
	condition:
		$pattern
}
...
rule sched_getaffinity_8dd8ce3612642ae17707f69b26507ec8 {
	meta:
		aliases = "sched_getaffinity"
		size = "94"
		objfiles = "sched_getaffinity@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 53 8B 7C 24 10 8B 74 24 14 89 F1 85 F6 79 05 B9 FF FF FF 7F 8B 54 24 18 53 89 FB B8 F2 00 00 00 CD 80 5B 89 C3 3D 00 F0 FF FF 76 0E E8 ?? ?? ?? ?? F7 DB 89 18 83 C8 FF EB 1E 83 C8 FF 83 FB FF 74 16 29 DE 56 6A 00 8B 44 24 20 01 D8 50 E8 ?? ?? ?? ?? 31 C0 83 C4 0C 5B 5E 5F C3 }
	condition:
		$pattern
}

rule __GI_lfind_e8a547293c19b88a736e3891c2dbd281 {
	meta:
...
rule wctrans_fb8a97cdc5747e98711eab6a457c943e {
	meta:
...
rule __GI_strndup_694d743242225696fd0150ec1e0f00ab {
	meta:
...
rule __GI_fwrite_unlocked_74f714be1b7cad710df9617e20a9e6de {
	meta:
...
rule __GI_strcat_fc39f2e68c3e2b9572ead413f87d6dfc {
	meta:
...
rule __GI_memcpy_55b9f3edcc52a715897dab39ce8dbfc0 {
	meta:
...
rule mq_open_c61d9cdc7e043bc7eebe202a89eb752b {
	meta:
		aliases = "mq_open"
		size = "95"
		objfiles = "mq_open@librt.a"
	strings:
		$pattern = { ( CC | 57 ) 56 83 EC 04 8B 7C 24 10 8B 4C 24 14 80 3F 2F 74 0D E8 ?? ?? ?? ?? C7 00 16 00 00 00 EB 39 F6 C1 40 75 06 31 D2 31 F6 EB 0F 8B 54 24 18 8D 44 24 20 89 04 24 8B 74 24 1C 47 53 89 FB B8 15 01 00 00 CD 80 5B 89 C7 81 FF 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DF 89 38 83 C8 FF 5A 5E 5F C3 }
	condition:
		$pattern
}
...
rule mknodat_27b3820011e4bb980c4662c2317e199e {
	meta:
		aliases = "__GI_mknodat, mknodat"
		size = "72"
		objfiles = "mknodat@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 83 EC 08 8B 7C 24 14 8B 4C 24 18 8B 44 24 20 89 04 24 C7 44 24 04 00 00 00 00 8B 54 24 1C 89 C6 53 89 FB B8 29 01 00 00 CD 80 5B 89 C7 81 FF 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DF 89 38 83 C8 FF 5A 59 5E 5F C3 }
	condition:
		$pattern
}

rule tmpnam_62d6c72a7b2fed3f7209b8852d87fd9c {
	meta:
		aliases = "tmpnam"
		size = "81"
		objfiles = "tmpnam@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 83 EC 14 8B 7C 24 20 89 FE 85 FF 75 02 89 E6 6A 00 6A 00 6A 14 56 E8 ?? ?? ?? ?? 83 C4 10 85 C0 75 23 6A 03 56 E8 ?? ?? ?? ?? 5A 59 85 C0 75 15 85 FF 75 13 BF ?? ?? ?? ?? A5 A5 A5 A5 A5 BF ?? ?? ?? ?? EB 02 31 FF 89 F8 83 C4 14 5E 5F C3 }
	condition:
		$pattern
}
...
rule __GI_statfs64_316153a3b656783bdb27ac0809d5483c {
	meta:
...
rule fstatat64_1f93b9cbc9800e7ee4773ceffb55c968 {
	meta:
		aliases = "fstatat, fstatat64"
		size = "77"
		objfiles = "fstatat64@libc.a, fstatat@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 83 EC 60 8B 7C 24 6C 8B 4C 24 70 8B 74 24 78 89 E2 53 89 FB B8 2C 01 00 00 CD 80 5B 89 C6 3D 00 F0 FF FF 76 0E E8 ?? ?? ?? ?? F7 DE 89 30 83 CE FF EB 10 85 C0 75 0C FF 74 24 74 52 E8 ?? ?? ?? ?? 58 5A 89 F0 83 C4 60 5E 5F C3 }
	condition:
		$pattern
}
...
rule timer_settime_65ad4863c55c7e1f9d5b5824a2500189 {
	meta:
		aliases = "timer_settime"
		size = "57"
		objfiles = "timer_settime@librt.a"
	strings:
		$pattern = { ( CC | 57 ) 56 8B 4C 24 10 8B 54 24 14 8B 74 24 18 8B 44 24 0C 8B 78 04 53 89 FB B8 04 01 00 00 CD 80 5B 89 C6 81 FE 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DE 89 30 83 C8 FF 5E 5F C3 }
	condition:
		$pattern
}
...
rule __GI_sigprocmask_bf39f38edb163f87b6f052348f763d3a {
	meta:
		aliases = "sigprocmask, __GI_sigprocmask"
		size = "77"
		objfiles = "sigprocmask@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 56 8B 7C 24 0C 8B 4C 24 10 8B 54 24 14 85 C9 74 12 83 FF 02 76 0D E8 ?? ?? ?? ?? C7 00 16 00 00 00 EB 23 BE 08 00 00 00 53 89 FB B8 AF 00 00 00 CD 80 5B 89 C6 81 FE 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DE 89 30 83 C8 FF 5E 5F C3 }
	condition:
		$pattern
}
...
rule __GI_ioctl_c359df500a651d7c323acc5e7b6cc2d0 {
	meta:
		aliases = "ioctl, __GI_ioctl"
		size = "59"
		objfiles = "ioctl@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 83 EC 04 8B 7C 24 0C 8B 4C 24 10 8D 44 24 18 89 04 24 8B 54 24 14 53 89 FB B8 36 00 00 00 CD 80 5B 89 C7 81 FF 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DF 89 38 83 C8 FF 5A 5F C3 }
	condition:
		$pattern
}

rule fcntl64_a496320a67c0d8484db8e1de14cc1ca6 {
	meta:
		aliases = "__GI_fcntl64, fcntl64"
		size = "59"
		objfiles = "__syscall_fcntl64@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 83 EC 04 8B 7C 24 0C 8B 4C 24 10 8D 44 24 18 89 04 24 8B 54 24 14 53 89 FB B8 DD 00 00 00 CD 80 5B 89 C7 81 FF 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DF 89 38 83 C8 FF 5A 5F C3 }
	condition:
		$pattern
}
...
rule sysctl_9479f9d17e4b30919b9e0fee105459a8 {
	meta:
		aliases = "sysctl"
		size = "91"
		objfiles = "sysctl@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 83 EC 28 8B 44 24 30 89 04 24 8B 44 24 34 89 44 24 04 8B 44 24 38 89 44 24 08 8B 44 24 3C 89 44 24 0C 8B 44 24 40 89 44 24 10 8B 44 24 44 89 44 24 14 89 E7 53 89 FB B8 95 00 00 00 CD 80 5B 89 C7 81 FF 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DF 89 38 83 C8 FF 83 C4 28 5F C3 }
	condition:
		$pattern
}
...
rule reboot_5ff83088a84ddcb47b276181b225a440 {
	meta:
		aliases = "reboot"
		size = "50"
		objfiles = "reboot@libc.a"
	strings:
		$pattern = { ( CC | 57 ) 8B 54 24 08 BF AD DE E1 FE B9 69 19 12 28 53 89 FB B8 58 00 00 00 CD 80 5B 89 C7 81 FF 00 F0 FF FF 76 0C E8 ?? ?? ?? ?? F7 DF 89 38 83 C8 FF 5F C3 }
	condition:
		$pattern
}

rule __GI_close_a02017293cde43f33c14807fe58a266e {
	meta:
...
rule __GI_strlen_ed3aca4bf76582b787a24165936a35da {
	meta:
...
rule __GI_rawmemchr_9d200d63dcd2b2c62fedc0d7c3c4cada {
	meta:
...
rule __GI_read_e868081ec3e176b22088c20162f785d6 {
	meta:
...
rule __GI_execve_98ed76ca880027546f7bd8375d9eb19e {
	meta:
...
rule __GI_setitimer_8e33584d28bfaa2d013232fdc496d1c7 {
	meta:
...
rule localtime_7136705c105fe90cbbb2eabb88b6c03d {
	meta:
...
rule __GI_asctime_e214931d143cca1a32199edb31adc4a0 {
	meta:
		aliases = "hcreate, ether_aton, asctime, __GI_inet_ntoa, inet_ntoa, ether_ntoa, __GI_asctime"
		size = "17"
		objfiles = "asctime@libc.a, ether_addr@libc.a, inet_ntoa@libc.a, hsearch@libc.a"
	strings:
...
rule __GI_strtok_ce767ada53bd455cecd0660edb8a7c87 {
	meta:
		aliases = "gamma, strtok, __GI_lgamma, __ieee754_lgamma, lgamma, __GI_strtok"
		size = "22"
...
rule __GI_wcstoul_a324739ecc8f201fcb11afb9bebf3b72 {
	meta:
		aliases = "__GI_waitpid, waitpid, __GI_wcstoull, wcstoumax, strtoull, wcstoull, __GI_strtoull, strtoul, wcstoul, strtouq, __libc_waitpid, strtoumax, __GI_strtoul, wcstouq, __GI_wcstoul"
		size = "23"
		objfiles = "wcstoul@libc.a, strtoul@libc.a, wcstoull@libc.a, strtoull@libc.a, waitpid@libc.a"
	strings:
...
rule __GI_iswalnum_0410fd2bb80bbf0bc944c8d2d2133af7 {
	meta:
		aliases = "mktime, timelocal, mkstemp64, iswalnum, __GI_iswalnum"
		size = "14"
...
rule wcstoq_fe8e5520a3eef7a03748f1ff8e71080f {
	meta:
		aliases = "__GI_strtoll, strtoll, wcstoimax, __GI_strtol, wcstol, __GI_wcstol, strtoimax, strtoq, wcstoll, __GI_wcstoll, strtol, wcstoq"
		size = "23"
...
rule __GI_gethostbyname_60617713272f358c39f6fe181a7829f8 {
	meta:
		aliases = "__GI_iswalpha, gethostbyname, iswalpha, __GI_gethostbyname"
		size = "14"
...
rule atol_40657ce09f1f30b9225c7cb14e45ae11 {
	meta:
		aliases = "__GI_atoi, atoi, __GI_atol, atoll, atol"
		size = "17"
...
rule __initbuf_40ee71d690ba3f1ac166f398be907ae2 {
	meta:
		aliases = "__initbuf"
		size = "35"
		objfiles = "getproto@libc.a, getservice@libc.a"
	strings:
		$pattern = { ( CC | 83 ) 3D ?? ?? ?? ?? 00 75 19 68 8D 10 00 00 E8 ?? ?? ?? ?? A3 ?? ?? ?? ?? 5A 85 C0 75 05 E8 ?? ?? ?? ?? C3 }
	condition:
		$pattern
}

rule __GI___uClibc_init_57bd80c37f527c73ad3ba562800da4d5 {
	meta:
...
rule fgetpwent_55f8a496d4b651e19d576b899468cf9d {
	meta:
		aliases = "sgetspent, getgrnam, fgetspent, getspnam, getgrgid, fgetgrent, getpwnam, getpwuid, fgetpwent"
		size = "38"
		objfiles = "getgrnam@libc.a, getpwnam@libc.a, fgetspent@libc.a, getspnam@libc.a, getgrgid@libc.a"
	strings:
...
rule __GI_inet_makeaddr_85efe30aed676f0eb0798dc3958215ba {
	meta:
		aliases = "inet_makeaddr, __GI_inet_makeaddr"
		size = "82"
		objfiles = "inet_addr@libc.a"
	strings:
		$pattern = { ( CC | 83 ) EC 04 8B 4C 24 08 8B 54 24 0C 8B 44 24 10 83 FA 7F 77 0A 25 FF FF FF 00 C1 E2 18 EB 22 81 FA FF FF 00 00 77 0A 25 FF FF 00 00 C1 E2 10 EB 10 81 FA FF FF FF 00 77 08 25 FF 00 00 00 C1 E2 08 09 D0 89 04 24 8B 04 24 0F C8 89 01 89 C8 5A C2 04 00 }
	condition:
		$pattern
}
...
rule __GI_open64_6e27fa1a3b0055d4fad61e0c8063a637 {
	meta:
...
rule __GI_asprintf_3674412569e427ab0f649983548971e5 {
	meta:
		aliases = "asprintf, __GI_fscanf, syslog, __GI_fwprintf, swscanf, sscanf, fwprintf, __GI_syslog, __GI_fprintf, dprintf, __GI_sscanf, fwscanf, fscanf, fprintf, __GI_asprintf"
		size = "28"
		objfiles = "dprintf@libc.a, asprintf@libc.a, sscanf@libc.a, fscanf@libc.a, fwscanf@libc.a"
	strings:
...
rule __GI_gethostbyname2_c48f4ccac2008e169ebb41ba84a9b311 {
	meta:
...
rule __GI_getservbyport_5b0e5662a54c4d8934751769ff0efbd8 {
	meta:
...
rule __GI_getdtablesize_90b2f2c2661b18b66f57c4157bd76f83 {
	meta:
...
rule __GI_logb_a3671bd408dde00a7113e2fcab16cd08 {
	meta:
...
rule __GI___signbit_83d5f0e8f572d2ea1eb86ba5936ed863 {
	meta:
...
rule __GI___isinf_386028e5e6e9fa89335cefe6eb3ee9ed {
	meta:
...
rule atanhf_237deda38afdc1ba6e02b05f725d180f {
	meta:
		aliases = "rintf, fabsf, ceilf, asinhf, logf, expf, erfcf, coshf, exp2f, log1pf, truncf, cosf, log2f, logbf, tanf, sqrtf, significandf, asinf, sinhf, expm1f, lgammaf, acoshf, sinf, atanf, tanhf, cbrtf, log10f, gammaf, floorf, acosf, erff, roundf, atanhf"
		size = "27"
		objfiles = "gammaf@libm.a, exp2f@libm.a, tanf@libm.a, log2f@libm.a, fabsf@libm.a"
	strings:
...
rule __GI_setprotoent_4df162fa78943270536f2e1af5743cf4 {
	meta:
		aliases = "setservent, setnetent, setprotoent, __GI_setservent, __GI_setnetent, __GI_setprotoent"
		size = "106"
...
rule __GI_strtod_dabd39a3a649dc519b1084bccffdb2ef {
	meta:
...
rule __GI_sigblock_d231928d5e4b1454773d22f622fcb9fe {
	meta:
...
rule __GI_sigsetmask_c82be1d4f89c26bd5e34abb4b62d4de6 {
	meta:
...
rule __GI_copysign_b2b071703f26a313e88c9d6050f340df {
	meta:
...
rule copysignf_d4f7c0b3360f99708eca867df6e90175 {
	meta:
		aliases = "hypotf, atan2f, powf, fmodf, scalbf, remainderf, copysignf"
		size = "35"
		objfiles = "scalbf@libm.a, copysignf@libm.a, atan2f@libm.a, hypotf@libm.a, powf@libm.a"
	strings:
...
rule __GI_wcrtomb_eb0ee40fa19d3987fbf282b06a8ff151 {
	meta:
...
rule __ieee754_acosh_141a17fdb9e46e9b88155affcfed22a0 {
	meta:
...
rule __sysv_signal_b25eea1279522c03afbb328afe65df85 {
	meta:
...
rule __GI_svcerr_auth_95728fff2a3ddb48780e8f4fcb23a819 {
	meta:
...
rule __GI_log1p_519bd3c2d8515f0110a4f35332746da8 {
	meta:
...
rule __ieee754_exp_459a202c0c58edc95c1005e621e038c9 {
	meta:
...
rule __GI_endttyent_cfdf69f1557ffd5b30c45190c6a4399a {
	meta:
...
rule htonl_f0b2ac96b2a9c6535f6815b42030bed8 {
	meta:
		aliases = "ntohl, __GI_ntohl, __GI_htonl, htonl"
		size = "7"
...
rule __GI_pthread_attr_getdetachsta_f1910983a54cef9177564bec55e19797 {
	meta:
		aliases = "pthread_mutexattr_getkind_np, pthread_mutexattr_gettype, pthread_attr_getdetachstate, pthread_rwlockattr_getkind_np, __pthread_mutexattr_getkind_np, __pthread_mutexattr_gettype, __GI_pthread_attr_getdetachstate"
		size = "15"
...
rule svctcp_stat_e188e5a4fc3e86f0fc217b04ccc0272d {
	meta:
...
rule __GI_setenv_e63fbd4454ec064dc8f9d0b775e2be85 {
	meta:
		aliases = "svcfd_create, setenv, svcunixfd_create, __GI_setenv"
		size = "17"
...
rule __new_sem_init_d9a5d35f096e3b6f339c60eead2d3b9b {
	meta:
...
rule __GI_ffs_fa76d4818df8af69962f2ee9af2c2efb {
	meta:
...
rule xdrmem_getint32_25d25932753fcc2fe75ccaef4f4e35de {
	meta:
...
rule __GI_strnlen_71a22aeeede3454368cf6cbeb0891e42 {
	meta:
...
rule svctcp_freeargs_b00eac8f841dd8fed9c028bec12a6e98 {
	meta:
...
rule __GI_xdrmem_create_97696748e504083caad888c9e7749a71 {
	meta:
...
rule __GI_brk_4eaa515d503f2c4553c9f7f3a1b19ea7 {
	meta:
...
rule __setutent_d0091fc92c67932a0088f9f1a86f33ea {
	meta:
		aliases = "__setutent"
		size = "96"
		objfiles = "utent@libc.a"
	strings:
		$pattern = { ( CC | A1 ) ?? ?? ?? ?? 85 C0 79 49 68 02 00 08 00 FF 35 ?? ?? ?? ?? E8 ?? ?? ?? ?? A3 ?? ?? ?? ?? 5A 59 85 C0 79 1B 68 00 00 08 00 FF 35 ?? ?? ?? ?? E8 ?? ?? ?? ?? A3 ?? ?? ?? ?? 5A 59 85 C0 78 20 6A 01 6A 02 FF 35 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 0C C3 6A 00 6A 00 50 E8 ?? ?? ?? ?? 83 C4 0C C3 }
	condition:
		$pattern
}
...
rule xdr_void_62dea3be98d084788202ec199cbcebf6 {
	meta:
		aliases = "__GI_xdr_void, authnone_validate, __GI__stdlib_mb_cur_max, old_sem_extricate_func, floatformat_always_valid, _stdlib_mb_cur_max, xdr_void"
		size = "6"
		objfiles = "_stdlib_mb_cur_max@libc.a, oldsemaphore@libpthread.a, xdr@libc.a, floatformat@libiberty.a, auth_none@libc.a"
	strings:
...
rule svcudp_stat_cca2f00a582595d7f5454d6d2948b3e3 {
	meta:
		...
		size = "6"
		objfiles = "svc_authux@libc.a, svc_udp@libc.a, svc_unix@libc.a, svc_raw@libc.a, svc_tcp@libc.a"
	strings:
...
rule __errno_location_6df29da601a97e2ce0e822cd8a3c1c72 {
	meta:
		aliases = "__GI___errno_location, __res_state, __h_errno_location, __GI___h_errno_location, __libc_pthread_init, __errno_location"
		size = "6"
...
rule __GI_localeconv_3fd768a6d4d57dafe0ca2586f0809abb {
	meta:
...
rule noop_handler_2b6be1eb3c8d07c259f5da1535d26263 {
	meta:
		aliases = "clnttcp_abort, pthread_null_sighandler, __linuxthreads_death_event, _pthread_cleanup_pop_restore, authnone_destroy, __cyg_profile_func_exit, clntraw_destroy, __cyg_profile_func_enter, xdrmem_destroy, __linuxthreads_reap_event, clntunix_abort, authunix_nextverf, clntraw_abort, _pthread_cleanup_push_defer, __linuxthreads_create_event, __pthread_return_void, clntudp_abort, pthread_handle_sigdebug, clntraw_geterr, authnone_verf, svcraw_destroy, noop_handler"
		size = "1"
		objfiles = "clnt_udp@libc.a, clnt_unix@libc.a, events@libpthread.a, pthread@libpthread.a, xdr_mem@libc.a"
	strings:
		$pattern = { ( CC | C3 ) }
	condition:
		$pattern
...
rule xdr_longlong_t_22aa66de8a720f160758a4fe68aab955 {
	meta:
		aliases = "__GI_xdr_enum, __GI_xdr_int, __GI_setmntent, __pthread_initialize, xdr_int, setmntent, xdr_enum, xdr_u_longlong_t, vfork, xdr_u_int, rand, splay_tree_xmalloc_deallocate, partition_delete, __GI_cabs, cabs, __GI_xdr_u_int, mq_close, xdr_longlong_t"
		size = "5"
		objfiles = "mq_close@librt.a, pthread@libpthread.a, mntent@libc.a, rand@libc.a, xdr@libc.a"
	strings:
		$pattern = { ( CC | E9 ) ?? ?? ?? ?? }
	condition:
		$pattern
...
rule getwchar_d0a919a1423a1611f85f059ef10ad627 {
	meta:
		aliases = "_dl_app_fini_array, getwchar_unlocked, getwchar"
		size = "13"
...
rule __free_initshell_memory_ee810455a4c31ee784ee576a3760617f {
	meta:
		aliases = "__free_initshell_memory"
		size = "45"
		objfiles = "usershell@libc.a"
	strings:
		$pattern = { ( CC | FF ) 35 ?? ?? ?? ?? E8 ?? ?? ?? ?? C7 05 ?? ?? ?? ?? 00 00 00 00 FF 35 ?? ?? ?? ?? E8 ?? ?? ?? ?? C7 05 ?? ?? ?? ?? 00 00 00 00 58 5A C3 }
	condition:
		$pattern
}
...
rule vwprintf_8656ba417f55ecd54498627fc6f064b6 {
	meta:
		aliases = "vprintf, __GI_vscanf, vscanf, vwscanf, vwprintf"
		size = "23"
...
rule __GI_verrx_e995d28d2bc805c6592260c44650cadd {
	meta:
